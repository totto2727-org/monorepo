...
ブラウザがこの URL を取得すると、Vite が TS/JSX を変換し、`remix/ui` を `/node_modules/.vite/deps/remix_ui.js` に pre-bundle、HMR クライアントを注入します。変換後のエントリが `run` を呼び出します:
...
`vite build` で Worker バンドルとブラウザバンドルの両方を生成し、`wrangler deploy` で Cloudflare Workers に配布します。

ブラウザバンドルは本番ではハッシュ付きになります。現在の `Document` は `/app/assets/entry.ts` を直接参照しており dev では問題ありませんが、本番では Vite の manifest（または小さな manifest ヘルパー）でエントリパスを解決してから差し込む必要があります。dev のシンプルさを優先するため、これは TODO として残しています。
