...
**Persist the progress of the entire roadmap and its underlying milestones in `roadmap` in a machine-readable form**. Just as `progress.yaml` is the true source for a single cycle, `progress.yaml` functions as the true source at the roadmap level.

**Minimal responsibility**: the schema in this version holds only the "linkage between milestones ↔ oh-my-codingagent execution cycle `<identifier>`" and coarse-grained statuses (`planned` / `active` / `completed` / `blocked` / `cancelled`). It **does not hold fine-grained progress (current step name, gate state, detailed event history)**. When needed, fetch via `milestones[].workflow_identifiers[]` by traversing `docs/workflow/<identifier>/progress.yaml`.

## File location (1:1 correspondence exception)

`docs/roadmap/<roadmap-id>/progress.yaml`

`templates/progress.yaml` (extension `.yaml`) and this reference (`references/roadmap-progress-yaml.md`, extension `.md`) are the **third 1:1 correspondence exception**. Follows the same naming pattern as the existing exception (`templates/progress.yaml` ↔ `references/progress-yaml.md`). For details, see the 1:1 exception list in `share-artifacts/SKILL.md`.
...
- **Updater (Step 3):** the **Main of each underlying oh-my-codingagent execution cycle** (autonomous updates; the "`totto2727-dev-flow`-side update protocol" section of this document is normative)
...
milestones:
  - id: <milestone-id>
    ...
    workflow_identifiers: [] # linked oh-my-codingagent execution cycles (1:N allowed)
...
- **Append-only operation**: append the `<identifier>` at underlying oh-my-codingagent execution cycle start. High git 3-way merge auto-merge affinity
...
## `totto2727-dev-flow`-side update protocol

This section is the core operating rule of `progress.yaml`. A cycle whose `progress.yaml.roadmap` is non-null (= a oh-my-codingagent execution cycle linked to a milestone of the parent roadmap) is responsible for **autonomously updating** this file according to its own progression. The detailed protocol is written in the "`progress.yaml` update protocol" section (independent top level) of `totto2727-dev-flow/SKILL.md`, and this section restates it from 3 perspectives (what / when / how) on the `references/roadmap-progress-yaml.md` side.
...
Updating `progress.yaml` at each `totto2727-dev-flow` step completion is **not done in this version (scope out)**. Reasons:
...
#### Commit granularity (consistent with `totto2727-dev-flow/SKILL.md` commit conventions)

- `progress.yaml` update at **(a) cycle start**: **bundled** with the `progress.yaml` initialization commit (do not split into a separate commit)
- `progress.yaml` update at **(c) cycle completion**: **bundled** with the `totto2727-dev-flow` Step 9 Retrospective completion commit
- Example commit messages:
  - On start: `docs(totto2727-dev-flow/<identifier>): initialize cycle (linked to roadmap <roadmap-id> milestone <milestone-id>)`
  - On completion: `docs(totto2727-dev-flow/<identifier>): close cycle with retrospective (completed milestone <milestone-id> in roadmap <roadmap-id>)`
...
Because parallel oh-my-codingagent execution cycles may update the same `progress.yaml`, conflict mitigation is needed. This version **delegates to git 3-way merge** (adopting a minimal schema + simple merge policy: limit `status` scalar rewrites and `workflow_identifiers[]` to append-only, ensuring conflict resilience at the schema level):
...
3. Main does the following:
   ...
   4. Normal commit (`docs(roadmap/<roadmap-id>): resolve concurrent updates`)
...
- **Linkage:**
  ...
  - "`progress.yaml` update protocol" section in `totto2727-dev-flow/SKILL.md` (the source-of-truth of this protocol)
