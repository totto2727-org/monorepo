...
description: >
  [Shared by Main / Specialist] Provides the table of contents and specifications for all artifacts created in the delegated execution workflow.
...
For all artifacts created in the delegated execution workflow, this skill aggregates **authoring guides (`share-artifacts/references/`) and templates (`share-artifacts/templates/`)**.
...
- `references/roadmap-progress-yaml.md` ↔ `templates/progress.yaml` (the progress YAML for roadmap. The authoring guide is Markdown while the template is loaded as YAML, so the extensions differ. Same reason as `progress.yaml`.)
...
| 13  | `roadmap.md`                                                            | roadmap Step 1                                            | Main (roadmap Step 1 is Main-only)                                               | `share-artifacts/references/roadmap.md`               | `share-artifacts/templates/roadmap.md`               |
| 14  | `milestones/<milestone-id>.md`                                          | roadmap Step 2                                            | Main (roadmap Step 2 is Main-only)                                               | `share-artifacts/references/milestone.md`             | `share-artifacts/templates/milestone.md`             |
| 15  | `progress.yaml`                                                         | roadmap Steps 1-4                                         | Main (autonomously updated through both layers)                                  | `share-artifacts/references/roadmap-progress-yaml.md` | `share-artifacts/templates/progress.yaml`            |
| 16  | `docs/retrospective/roadmap-<roadmap-id>.md`                            | roadmap Step 4                                            | Main (roadmap Step 4 is Main-only)                                               | `share-artifacts/references/roadmap-retrospective.md` | `share-artifacts/templates/roadmap-retrospective.md` |
...
The artifacts and progress records of totto2727-dev-flow are **aggregated under a directory dedicated to each cycle** and committed to the repository. This makes **interruption and resumption** of work possible (a different session can read the same directory and grasp the current location).
...
graph LR
    ...
    RP["progress.yaml<br/>(updated continuously through Steps 1-4)"]
...
The roadmap retrospective from `roadmap` Step 4 is also stored in the same aggregated directory as `docs/retrospective/roadmap-<roadmap-id>.md` (the `roadmap-` prefix avoids name clashes with the `<identifier>.md` files of the underlying oh-my-codingagent execution cycles). For details, see `share-artifacts/references/roadmap-retrospective.md`.

#### Temporary report for in-progress user inquiries

- **Storage location:** `$TMPDIR/totto2727-dev-flow/<phase>-<step>-<purpose>.md`
...
- **Detailed usage:** see "Basic policies" section in `totto2727-dev-flow` (Artifact-as-Gate-Review / Report-Based Confirmation for In-Progress Questions)
...
- **Volatile output destination:** `$TMPDIR/totto2727-dev-flow/<identifier>-pr-body.md` (regenerated by Main immediately before sending it via `gh pr edit --body-file`)
...
- **PR / CI operations procedure:** delegated to oh-my-codingagent; this plugin only retains the document format.
...
2. It copies `share-artifacts/templates/<name>.md` into the appropriate roadmap or execution artifact path.
...
Artifacts must be reflected in the repository at step completion. For details, see "Step completion commit conventions" in `totto2727-dev-flow`.
...
- "Start Steps 1-5 (Intent Clarification through Task Decomposition)" → `totto2727-dev-flow`
