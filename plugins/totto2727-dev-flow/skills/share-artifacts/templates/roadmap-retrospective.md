...
This document is the **retrospective for the roadmap as a whole**, drafted during `roadmap` **Step 4 (Roadmap Retrospective)**. It aggregates each underlying workflow-level execution's retrospective or summary and adds roadmap-specific themes (a summary of milestone completion / soundness of the dependency graph / improvement proposals specific to roadmaps). Place it at `docs/retrospective/roadmap-<roadmap-id>.md` (aggregation form, with the `roadmap-` prefix). See `share-artifacts/references/roadmap-retrospective.md` for authoring details.
...
| Milestone ID       | Title                 | Final Status           | Rationale for the completion judgment | Related workflow execution `<identifier>` |
| ------------------ | --------------------- | ---------------------- | ------------------------------------- | ----------------------------------------- |
...
- Missing dependencies discovered during execution (milestone C implicitly required D, but the absence of that dependency forced rework in later executions)
- Effective vs. theoretical parallelism (theoretical parallelism vs. the actual number of executions launched in parallel)

## Aggregation of underlying workflow-level executions

{{workflow_aggregation}}

Summarize each underlying workflow-level execution's retrospective or summary in **one paragraph per execution**. Each paragraph must include:

- The execution `<identifier>`
- The id of the milestone it is attached to
- Up to 1-2 standout "what went well" / "issues" items observed in that cycle alone
- A link to that execution's retrospective or summary
...
<!-- Repeat for the number of underlying executions -->

## Improvement proposals specific to the roadmap layer

{{roadmap_specific_improvements}}

Improvement proposals for the roadmap layer (the `roadmap` skill / the `progress.yaml` schema / milestone decomposition granularity, etc.). Improvement proposals that are self-contained inside an underlying execution are already written in that execution's retrospective or summary, so this section contains only the strategic-level improvements derived from aggregating those.
...
### Retrospective on the `roadmap` ↔ workflow-level execution integration protocol
...
- Number of underlying workflow-level executions: {{workflow_count}}
