...
- [`ts-effect-latest-api.md`](./ts-effect-latest-api.md) — Raw upstream Effect Smol `MIGRATION.md` and `packages/effect/SCHEMA.md` content. Use when checking latest Effect v4 API names, migration notes, package organization, or Schema API details.
...
- [`ts-effect-remix.md`](./ts-effect-remix.md) — Remix `clientEntry` / `on(...)` client event patterns with Effect. Use when writing browser event handlers that perform HTTP calls, state mutation, WebAuthn, or navigation.
- [`ts-totto2727-fp.md`](./ts-totto2727-fp.md) — `@totto2727/fp` package usage. Use when adding `@totto2727/fp` as a dependency, importing its modules (Effect / option-t bridges, FP utilities), or defining domain error payloads / caught-error propagation.
