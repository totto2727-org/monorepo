# MoonBit Conventions Index

Tier-2 index for MoonBit conventions in the `coding` skill.

## In-plugin detail files

- [mbt-bestpractice.md](./mbt-bestpractice.md) — MoonBit coding standards (naming, idioms, error handling, traits, etc.). Use when writing or reviewing MoonBit code.
- [mbt-cli-application.md](./mbt-cli-application.md) — MoonBit CLI application structure. Use when implementing `admiral`-based CLI commands, option parsing, request body types, and JSON response handling.

## Related references

- For MoonBit language reference (syntax / types / functions / methods), see [docs-moonbit](../../docs-moonbit/SKILL.md).
- For MoonBit testing standards, see [test](../../test/SKILL.md) (sibling skill in this plugin).
