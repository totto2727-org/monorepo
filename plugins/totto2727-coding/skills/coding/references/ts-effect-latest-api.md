# Latest Effect API Raw Reference

This file intentionally vendors raw upstream Markdown from Effect Smol so agents can inspect the latest Effect v4 API notes without relying on stale local summaries. Refresh from the source URLs when updating Effect.
