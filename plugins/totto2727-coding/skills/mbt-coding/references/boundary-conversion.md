...
Inside a manual `FromJson` implementation, use `totto2727/lens` for known object-field selection. Define complete lenses once at the top level with explicit types, use only their `get` operations inside functions, and never construct or compose a lens in a function body. Keep lens reads in the `FromJson` implementation or in a private implementation-only helper, and translate `LensError` to `JsonDecodeError`. Use direct `@json.from_json` or pattern matching for scalar wrappers and discriminators. Outside `FromJson`, lens access is allowed only for values intentionally kept as unstructured `Json` because their shape is dynamic or lacks a stable type. A known structure must still cross the boundary through `FromJson`.
