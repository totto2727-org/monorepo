...
- **CI 結果の集計が SC に含まれる場合** (例: SC-7 「各ステップ完了コミットの CI が最終 PASS」相当): `ci-monitoring` スキルの read 系手順 (`gh run list --branch <branch> --commit <sha> --json conclusion`) を使って実測する。`gh pr view --json` / `gh pr list --json` / `gh run list --json` / `gh run view --json` のみ Specialist として使用可 (write 系は Main 専属、`specialist-common §7` 参照)
