...
- **CI 結果の集計が SC に含まれる場合** (例: SC-7 「各ステップ完了コミットの CI が最終 PASS」相当): `ci-monitoring` スキルの read 系手順 (`gh run list --branch <branch> --commit <sha> --json conclusion`) を使って実測する
- **PR 状態の集計が SC に含まれる場合** (例: 「PR が Draft 作成された」「`isDraft: false` で Ready 化された」「description が更新された」等): `pr-manager` スキル §4 の read 系手順を使って実測する
