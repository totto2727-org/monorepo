...
dev-workflow ワークフローで作成される全成果物について、**書き方ガイド（`shared-artifacts/references/`）とテンプレート（`shared-artifacts/templates/`）**を集約する。
...
- 各成果物は `docs/dev-workflow/<identifier>/` 配下に保存される（詳細: `dev-workflow` の「成果物保存構造」）
...
| 2   | `intent-spec.md`              | Step 1         | `intent-analyst`                       | `shared-artifacts/references/intent-spec.md`        | `shared-artifacts/templates/intent-spec.md`        |
| 3   | `research/<topic>.md`         | Step 2         | `researcher` (観点ごと並列)            | `shared-artifacts/references/research-note.md`      | `shared-artifacts/templates/research-note.md`      |
| 4   | `design.md`                   | Step 3         | `architect`                            | `shared-artifacts/references/design.md`             | `shared-artifacts/templates/design.md`             |
| 5   | `qa-design.md`                | Step 4         | `qa-analyst`                           | `shared-artifacts/references/qa-design.md`          | `shared-artifacts/templates/qa-design.md`          |
| 6   | `qa-flow.md`                  | Step 4         | `qa-analyst`                           | `shared-artifacts/references/qa-flow.md`            | `shared-artifacts/templates/qa-flow.md`            |
| 7   | `task-plan.md`                | Step 5         | `planner`                              | `shared-artifacts/references/task-plan.md`          | `shared-artifacts/templates/task-plan.md`          |
| 8   | `TODO.md`                     | Step 6〜7 全体 | (Main が維持、`task-plan.md` から生成) | `shared-artifacts/references/todo.md`               | `shared-artifacts/templates/TODO.md`               |
| 9   | `implementation-logs/<id>.md` | Step 6         | `implementer` (タスクごと並列)         | `shared-artifacts/references/implementation-log.md` | `shared-artifacts/templates/implementation-log.md` |
| 10  | `self-review-report.md`       | Step 7         | `self-reviewer`                        | `shared-artifacts/references/self-review-report.md` | `shared-artifacts/templates/self-review-report.md` |
| 11  | `review/<aspect>.md`          | Step 8         | `reviewer` (観点ごと並列)              | `shared-artifacts/references/review-report.md`      | `shared-artifacts/templates/review-report.md`      |
| 12  | `validation-report.md`        | Step 9         | `validator`                            | `shared-artifacts/references/validation-report.md`  | `shared-artifacts/templates/validation-report.md`  |
| 13  | `retrospective.md`            | Step 10        | `retrospective-writer`                 | `shared-artifacts/references/retrospective.md`      | `shared-artifacts/templates/retrospective.md`      |
...
- Specialist はテンプレートをコピーして `docs/dev-workflow/<identifier>/` 配下に配置し、プレースホルダを埋める
...
2. `shared-artifacts/templates/<name>.md` をコピーして `docs/dev-workflow/<identifier>/<artifact>.md` に配置
...
│   └── <task-id>.md
├── self-review-report.md      # Step 7 成果物
├── review/                    # Step 8 成果物（観点ごとにファイル分割）
│   ├── security.md
│   ├── performance.md
│   └── ...
├── validation-report.md       # Step 9 成果物
├── validation-evidence/       # Step 9 の大きな証跡（必要な場合のみ）
│   └── <evidence-file>
└── retrospective.md           # Step 10 成果物
```
...
以下はサイクル作業ディレクトリ (`docs/dev-workflow/<identifier>/`) **外**に保存される:
...
2. Specialist が `shared-artifacts/templates/<name>.md` をコピーして `docs/dev-workflow/<identifier>/<artifact>.md` に配置
...
別セッション／別ユーザーが中断済みサイクルを再開する際、`docs/dev-workflow/<identifier>/` 配下のファイル群だけで文脈を完全復元できる設計になっている。再開プロトコルの詳細は `dev-workflow` の「セッション再開時」を参照。
