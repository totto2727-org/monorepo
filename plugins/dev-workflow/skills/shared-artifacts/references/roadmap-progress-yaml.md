...
並行する複数の `dev-workflow` サイクルが同一 `roadmap-progress.yaml` を更新しうる構造のため、衝突対策が必要。本バージョンは **git 3-way merge 任せ** (案 A、Research progress-yaml-concurrency / design.md 確定 5 参照):
