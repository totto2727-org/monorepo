...
`design.md` を**実装可能な粒度のタスク**に分解し、依存関係・並列性・見積りを明示する。Step 5 開始後は**不変な計画書**として扱い、実行中の状態追跡は `TODO.md` で行う。

## 作成者 / 作成タイミング

- **作成者:** `planner` Specialist（単一インスタンス）
- **作成ステップ:** Step 4 (Task Decomposition)
- **承認:** ユーザー承認必須（実装開始の合意ゲート）
- **変更:** Step 5〜6 中を通じて**常に不変**。追加タスクは `TODO.md` の「後発追加タスク」セクションにのみ追記し、差分理由も TODO.md 冒頭に記録する（`task-plan.md` 本体は書き換えない）。追加タスクが多発する場合は Step 4 へのロールバックを検討
...
- **不変性:** 確定後は書き換えない。Step 5〜6 中の追加タスクは `TODO.md` 側で管理
