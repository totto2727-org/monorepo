...
- 規範的制約: セキュリティ・コンプライアンス・既存横断 ADR・上位プロダクト方針
...
## 仮想マイルストーン分解の説明性 (SC-11 への適合)

本ドキュメントを読んだ Main / ユーザーが、**任意の 1 つの仮想ゴール (例: "OAuth 認証導入") を入力として、3 つ以上のマイルストーン分解と各マイルストーンの定性的到達点・依存関係を抽出する手順**を、追加情報なしで再現できる必要がある (Intent Spec SC-11)。`roadmap.md` を書くときは以下を満たすことで説明性を確保する:
...
4. **マイルストーン一覧に最低 3 行**: 「3 件以上に分解可能」という SC-11 の条件を直接示す
