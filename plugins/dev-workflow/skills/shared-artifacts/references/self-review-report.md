...
implementer チームが生成した**全 diff を統合的にレビュー**し、外部レビュー前に明らかな問題（`design.md` 違反、Intent Spec 未達見込み、明白な bug、テスト網羅不足等）を潰す。Step 7 の外部観点レビューとは別層。

## 作成者 / 作成タイミング

- **作成者:** `self-reviewer` Specialist（単一インスタンス）
- **作成ステップ:** Step 6 (Self-Review)
