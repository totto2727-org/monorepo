...
Intent Spec の成功基準を**観測可能なテストケース集合**へと展開する。各テストケースには「実行主体」「検証スタイル」の 2 軸を独立に付与し、特定のテストフレームワーク (Vitest / Playwright / pytest 等) に依存しない抽象レベルで記述する。Step 6 (Implementation) の `implementer` がこのドキュメントを参照してテストを実装し、Step 9 (Validation) の `validator` がカバレッジを実測する。
...
- `実装状況` (Step 9 で埋まる: `pending` / `implemented` / `passed` / `failed`)
...
成功基準 → TC-ID の逆引き表。Step 9 validator がカバレッジ確認に使用する。
...
- **出力先:** `task-plan.md` (任意で TC-ID 紐付け), `code` (Step 6 implementer がテスト実装), `validation-report.md` (Step 9 validator がカバレッジ実測)
