...
このサイクルで起票した、または既存の ADR (General mode `docs/adr/` / Roadmap mode `docs/roadmap/<roadmap-id>/adr/`) があればリンクする。サイクル固有の判断はここではなく本ドキュメント内で完結させる (モード判定の詳細: `adr/SKILL.md` の「モード判定フロー」)。
