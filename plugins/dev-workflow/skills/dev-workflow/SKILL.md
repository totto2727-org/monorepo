...
**成果物:** 観点ごとに `docs/dev-workflow/<identifier>/review/<aspect>.md` (指摘 / 深刻度 (Blocker / Major / Minor) / 該当箇所 (コミット SHA + ファイル行番号) / 推奨アクション)。`<aspect>` ∈ {`security`, `performance`, `readability`, `test-quality`, `api-design`, `holistic`} (+ プロジェクト固有追加観点)

**Exit Criteria:**

- Blocker 指摘が 0 件
- Major 指摘は修正方針 (Step 6 戻し / Retrospective 繰越) がユーザー判断で確定
...
**Gate:** ユーザー承認必須 (Blocker 0 件の確認と、Major/Minor 指摘の扱い方針)

**失敗時 / ロールバック:**

- Blocker 指摘発生 → Step 6 に戻り修正 (再活性化時は新規 `implementer` 起動)。戻り後は Step 6 → Step 7 を再実行 (既存 reviewer インスタンスに差し戻し)
...
- **Blocker 指摘が繰り返し発生 → 設計レベルの問題の可能性**。Step 3 への回帰を In-Progress ユーザー問い合わせ形式で相談 (本セクション末尾の「ループ上限の目安」参照)

#### Step 6 ↔ Step 7 ループ

Blocker 指摘があった場合のループ構造。Step 6 と Step 7 は別ステップなので、ステップを跨ぐ Specialist 再利用は行わないが、**各ステップの活性化期間中はインスタンスを維持**する。
...
[Step 7 活性化] reviewer B1..B6 並列起動
   (security / performance / readability / test-quality / api-design / holistic)
    ↓
    各 reviewer が観点別 Review Report を生成
    ↓
    ...
    └── 1 件以上 Blocker あり
        ...
        (B1..B6 は Step 7 継続のため維持)
        C1..Ck が修正 diff を返却
            ↓
        [Step 6 再び完了] C1..Ck 役割終了
            ↓
        既存 B1..B6 が再レビュー (差し戻し、再 Round)
            ↓
        Blocker 0 件確認 → Step 8 へ (B1..B6 はここで役割終了)
```

**ループ上限の目安:** 同一 Round 系列で全観点合算の Blocker 指摘が 3 周以上発生する場合、設計レベルの問題を疑い Step 3 へロールバックを検討する。ループのたびに進捗記録へ回数と原因を記録する。
