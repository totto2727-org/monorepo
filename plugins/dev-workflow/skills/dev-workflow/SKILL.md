...
- review レポートは**観点ごとに 1 ファイル** (`review/<aspect>.md`) で保持し、Round が増えても**同一ファイルに `## Round N (date)` 見出しで追記**する
- 別ファイル (例: `review/round2-<aspect>.md`) を作らない: 1 ファイルで「Round 1 で何を指摘したか / Round 2 でどう解消したか / 新規発生は何か」を時系列追跡できる構造を維持
- 各 Round のセクションには **前 Round 指摘ごとの最終判定** を必ず明示する。判定ラベルは以下から選ぶ:
  - **`fixed`**: 該当 Round の修正で解消済 (修正 commit SHA を併記)
  - **`partial`**: 一部解消、残課題あり (残課題内容と次 Round への持ち越し可否を記述)
  - **`pending`**: 未解消で次 Round で対応予定 (担当タスクまたは追加タスク ID を記述)
  - **`accepted-as-is`** (= 「無視」): ユーザー承認ゲートで「Retrospective 繰越合意 / 対応不要」と確定したため Step 7 内では対応せず受容 (合意日付と承認者を併記)
  - **`obsolete`**: 設計変更や前提変更により当該指摘が無効化 (無効化理由を記述)
- 各 Round のセクションには「現 Round で新規発生した指摘」を別カテゴリで追記する (旧指摘と混在させない)
- ファイル末尾には「現 Round の Gate 判定 (`approved` / `needs_fix`)」を残す
- これにより 1 ファイルで「対応した指摘」「無視 (受容) した指摘」「未解消で持ち越し中の指摘」が時系列で追跡可能になる
