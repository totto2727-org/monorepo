...
具体的なコマンド・冪等性ガード (`gh pr list --head` での既存確認 → `gh pr create --draft --body-file`) は **`pr-manager` スキル §1** に集約している。本ワークフローは「Step 1 完了直後に Main が呼び出す」ことのみを規定する。

PR description テンプレートは `shared-artifacts/templates/pr-body.md`、書き方ガイドは `shared-artifacts/references/pr-body.md`。Main は揮発ファイル `$TMPDIR/dev-workflow/<identifier>-pr-body.md` に再生成して送信する (リポジトリ内には永続化しない)。

### PR 概要更新 (各ステップ完了時)

PR 概要 (PR description) は **各ステップ完了時に必ず更新する**。さらに、ステップ途中であっても内容に変化があれば**適宜**更新してよい (例: Research Note の追加、Blocker 発生時のステータス反映など)。

具体的なコマンド (`gh pr edit <num> --body-file <path>`、`--body-file` 必須化) は **`pr-manager` スキル §2** に集約している。本ワークフローは「いつ更新するか (= 各ステップ完了コミット直後 + 適宜)」を規定する。

#### PR description テンプレート

サイクル進行に応じて段階的に埋まる構造。テンプレート本体は `shared-artifacts/templates/pr-body.md`、書き方ガイドは `shared-artifacts/references/pr-body.md` に切り出している。Main は揮発ファイル `$TMPDIR/dev-workflow/<identifier>-pr-body.md` にコピーして埋め、`pr-manager` §2 の手順で送信する (リポジトリ内には永続化しない)。

Step 1 完了時には Summary + Cycle artefacts (intent-spec のみ) + Progress checklist (Step 1 のみ `[x]`) + CI status (初回コミットの結果) で初期化し、以降のステップ完了ごとに該当セクションを更新する。各セクションの具体仕様 (CI status の埋め方、Notable incidents の省略基準、Test plan の引用元など) は `shared-artifacts/references/pr-body.md` の「セクション仕様」を参照。

### バックグラウンド CI watch + 結果取得 (各ステップ完了コミット直後)

各ステップ完了コミット (Step 6 はタスク単位コミット) を push した直後、当該コミットに紐付く CI run が **PASS するまで当該ステップを完了と認めない**。CI 確認の具体手順は **`ci-monitoring` スキル** に集約されており、本ワークフローはそこから手順を呼び出す。

**必須手順 (詳細は `ci-monitoring` スキル参照):**

1. push 直後に `gh run list --branch <branch> --commit <sha> --workflow CI --json databaseId` で run id を取得 (race 回避ループあり、最大 30 秒)
2. バックグラウンドで `gh run watch <run-id> --exit-status --interval 10 --compact > "$TMPDIR/dev-workflow/ci-watch-<run-id>.log" 2>&1` を起動。終了直後に `echo "EXIT=$?" >> LOG` で **EXIT 行を必ず記録**
3. **二重チェックで PASS 判定**: log 末尾の `EXIT=` 行 と `gh run view <run-id> --json conclusion --jq '.conclusion'` の **両方が `success`** で初めて PASS と判定する。バックグラウンドタスクの完了通知 (= bash の exit code) だけで判定してはならない (過去サイクル `2026-05-03-pr-ci-integration` で 9 件の CI failure 見落とし事故あり)
4. CI 待機中も Main は次の準備作業 (例: 次ステップ Specialist の起動準備) を並行して進めてよい

#### CI 失敗時のリトライフロー (最大 2 回 → Blocker 化)

CI が失敗した場合、Main は最大 **2 回までリトライ**する。本リポの直近 CI 失敗事例は決定的な問題 (oxfmt Formatting / typecheck) が大半であり、`gh run rerun` ではなく **修正コミットを新規 push する方式**を 1 リトライとカウントする。3 回目の失敗で **Blocker** 化し、In-Progress ユーザー問い合わせ形式でユーザー判断を仰ぐ (作業中断)。具体的なループ構造とコマンド例は `ci-monitoring` スキル §4 参照。
...
具体的なコマンド・冪等性ガード (`gh pr view --json isDraft` で `true` を確認 → `gh pr ready` 実行、`false` なら no-op) は **`pr-manager` スキル §3** に集約している。本ワークフローは「Step 9 Retrospective コミット + CI PASS 確認後に Main が呼び出す」ことのみを規定する。

Ready 化のタイミングは Step 9 (Retrospective) コミット以降であることが必須 (検証手段は `pr-manager` §3 末尾参照)。
