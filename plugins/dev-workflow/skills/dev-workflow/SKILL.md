...
description: >
  ...
  各 Specialist の作業詳細は specialist-* スキルに、成果物仕様は share-artifacts
  スキルに委譲する。
  ...
  (specialist-* スキルを参照)、成果物仕様の参照のみ (share-artifacts スキル)。
---
...
| 3    | Design               | `architect` × 1                | User | `design.md` (+ サイクル境界を越える判断は ADR を併発)  | `specialist-architect`            |
...
6. **サイクル内で完結しない判断 (現サイクルを跨いで影響する判断)** が発生した場合のみ、`share-adr` skillを使って別途 ADR を起票 (起票モードは `share-adr/SKILL.md` の「モード判定フロー」に従う。後述「ADR の起票条件」も参照)

**成果物:** `docs/workflow/<identifier>/design.md` (設計目標 / アプローチ / コンポーネント構成 / データフロー / 代替案比較 / 拡張ポイント / 運用上の考慮事項 / 起票した ADR へのリンク)

**Exit Criteria:**

- `design.md` + `progress.yaml` がコミット済み (ADR を起票した場合は ADR 本体も同コミット)
...
`design.md` とは別に ADR を起票するのは、**現サイクル内で完結せず、他のサイクル / 他のロードマップ / プロジェクト全体に影響が及ぶ意思決定**に限る。`share-adr` skillは保存先の異なる 2 モード (General / Roadmap) を持ち、Main が起票時にモードを判定する (詳細: `share-adr/SKILL.md` の「モード判定フロー」)。

| 起票する (サイクル境界を越える)                                                          | 起票しない (サイクル内完結)                 |
| ---------------------------------------------------------------------------------------- | ------------------------------------------- |
| 「プロジェクト全体で Effect を採用」(General)                                            | 「この機能のキャッシュ戦略を LRU にする」   |
| 「全サービスで gRPC を使う」(General)                                                    | 「この API のページネーションは cursor 型」 |
| 「認可レイヤを OpenFGA に統一」(General)                                                 | 「この画面のバリデーションは zod で書く」   |
| 「独立した複数 dev-workflow サイクルが共有するキャッシュ層分離方針」(General)            | 「このサイクルで使う一時キーの命名規則」    |
| 「`oauth-rollout` ロードマップ配下の全サイクルが共有する `AuthSession` 型定義」(Roadmap) | 「このサイクルでの Session ガード文言」     |

判定の早見:

- **General mode (`docs/adr/<YYYY-MM-DD-title>.md`)**: roadmap 文脈なし、または複数 roadmap / 独立した複数 dev-workflow サイクル / プロジェクト全体に効く判断
- **Roadmap mode (`docs/roadmap/<roadmap-id>/adr/<YYYY-MM-DD-title>.md`)**: 単一 roadmap 配下の複数 dev-workflow サイクルが共有する文脈・規範 (現サイクルが roadmap 配下で起動された場合に使用候補となる。`progress.yaml.roadmap.id` で roadmap 文脈の有無を判定)

ADR を起票した場合: `progress.yaml.artifacts.external_adrs` にパス追記 (General / Roadmap mode 両方を同列に列挙、scope は ADR 本体の frontmatter で識別)、`design.md` から該当 ADR にリンク。
...
1. `qa-analyst` に Intent Spec、`design.md`、`share-artifacts/references/qa-design.md` / `qa-flow.md`、`templates/qa-design.md` / `qa-flow.md`、関連プロジェクト言語固有テストスキル (TS なら `vite-plus`、MoonBit なら `moonbit-bestpractice` 等) を渡して起動
...
- 詳細は `share-artifacts/references/qa-design.md` および `qa-flow.md` を参照
...
**フォーマット詳細:** `share-artifacts/references/todo.md` + `share-artifacts/templates/TODO.md`。
...
**reviewer 成果物のファイル運用 (本ファイルでは規約のみ規定、書式詳細は `share-artifacts/references/review-report.md` に委譲):**
...
- ファイル構造、状態ラベル (`fixed` / `partial` / `pending` / `accepted-as-is` / `obsolete`)、列構成、Round 履歴メタの位置づけ等の**書式詳細は `share-artifacts/references/review-report.md` および `share-artifacts/templates/review-report.md` を真のソースとする** (本ファイルには書式仕様を持たない、二重管理回避)
...
1. `retrospective-writer` に全ステップの進捗記録 / 全成果物 (Intent Spec / Research Notes / Design Document / Task Plan / diff / 各種 Report / 起票した ADR (General / Roadmap mode どちらも) があれば) / Blocker 履歴 / ループ回数 (Step 6 ↔ Step 7 の往復、Step 7 の差し戻し回数) / ユーザー承認ゲートでのやり取りを渡す
...
**ライフサイクル:** retrospective.md は **次サイクルが改善案項目を消化した時点で削除する** 揮発的な報告ボックス。永続記録すべき判断は ADR (General mode `docs/adr/` または Roadmap mode `docs/roadmap/<roadmap-id>/adr/`) に切り出す。**ADR (永続) と `docs/retrospective/` (揮発)** の役割分担。
...
本プラグインでは**成果物仕様を `share-artifacts` スキルに集約**することで、Main / Specialist / ユーザーの全ステークホルダーが同じ真のソースを参照できるようにしている。テンプレートと書き方ガイドの 1:1 対応が担保され、仕様変更時の更新漏れを防げる。

Main は以下を参照する:

- **成果物一覧とテンプレートパス**: `share-artifacts` SKILL.md の「成果物一覧 (目次)」
- **ディレクトリ構造** (`docs/workflow/<identifier>/` 配下): `share-artifacts` SKILL.md の「成果物保存構造」
- **`<identifier>` の命名ルール**: `share-artifacts` SKILL.md の「`<identifier>` の命名ルール」
- **サイクル外成果物** (ADR (General / Roadmap mode)、一時レポート): `share-artifacts` SKILL.md の「サイクル外の成果物」
- **各成果物の書き方**: `share-artifacts/references/<name>.md` (1:1 対応)
- **`progress.yaml` のスキーマ・書き方**: `share-artifacts/references/progress-yaml.md`
- **`design.md` と ADR の使い分け**: `share-artifacts/references/design.md`
- **`TODO.md` と内部タスクリストの同期**: `share-artifacts/references/todo.md`

Specialist 起動時、Main はテンプレートパス (`share-artifacts/templates/<name>.md`) と reference パス (`share-artifacts/references/<name>.md`) の**両方を入力に含める**こと。Specialist はテンプレートを埋める際、必ず対応する reference を参照する。
...
- ADR (General mode `docs/adr/<file>.md` または Roadmap mode `docs/roadmap/<roadmap-id>/adr/<file>.md`) を起票した場合は、該当 ADR ファイルも同じコミットに含めてよい (`design.md` の更新とセットで)
...
| トリガー                                                           | アクション                                               | 委譲先               |
| ------------------------------------------------------------------ | -------------------------------------------------------- | -------------------- |
| サイクル初期化コミット `docs(dev-workflow/<id>): initialize cycle` | Draft PR を 1 件作成 (既存 PR があれば再利用、冪等)      | `share-pr-manager` §1      |
| 各ステップ完了コミット直後 + 内容変化時に適宜                      | PR description を再生成して送信                          | `share-pr-manager` §2      |
| 各ステップ完了コミット (Step 6 はタスク単位コミット) を push 直後  | CI run が PASS するまで当該ステップを完了と認めない      | `share-ci-monitoring` §2-3 |
| CI 失敗時                                                          | 修正コミット push を 1 リトライとして最大 2 回 → Blocker | `share-ci-monitoring` §4   |
| Step 9 (Retrospective) 完了コミット + 該当 CI PASS 後              | Draft → Ready 化 (`isDraft` 事前確認、冪等)              | `share-pr-manager` §3      |

**判定基準と方針** (本ワークフロー固有のため本セクションに残す):

- **CI PASS は二重チェック**: `gh run watch` の log 末尾 `EXIT=` 行と `gh run view --json conclusion` の両方が `success` で初めて PASS と判定する (`share-ci-monitoring` §3 必須手順)。バックグラウンドタスクの完了通知 (= bash の exit code) のみで判定してはならない
- **責任所在**: PR / CI の write 系 `gh` コマンドは **Main 専属**。Specialist は read 系のみ (`specialist-common` §7 / `share-pr-manager` §5 / `share-ci-monitoring` 基本方針と一致)
...
詳細手順は各専門スキル (`share-pr-manager` / `share-ci-monitoring` / `share-artifacts/{templates,references}/pr-body.md`) を参照。
...
- 現サイクルを跨いで影響する意思決定 (複数 roadmap / 独立した複数 dev-workflow サイクル / プロジェクト全体に及ぶもの、または単一 roadmap 配下の複数サイクルが共有する規範) の記録 → `share-adr` skillに委譲 (General / Roadmap mode の使い分けは `share-adr/SKILL.md` の「モード判定フロー」参照。Design Document の代替にはならない)
...
- **CI run の確認・監視・リトライ手順** (`gh run watch` の二重チェック、修正コミット push でのリトライ、Blocker 化) → **`share-ci-monitoring`** スキルに委譲。本スキルは「いつ CI 確認を行うか (= 各ステップ完了コミット直後)」を規定し、「どう確認するか」は `share-ci-monitoring` 側で集約する
- **PR 操作の具体コマンド・冪等性ガード** (`gh pr create` / `gh pr edit` / `gh pr ready` / `gh pr view` / `gh pr list`) → **`share-pr-manager`** スキルに委譲。本スキルは「いつ PR 操作を行うか (= サイクル開始時 / 各ステップ完了直後 / Step 9 完了後)」を規定し、「どう操作するか」は `share-pr-manager` 側で集約する
- **PR description の雛形・各セクションの埋め方** → `share-artifacts/{templates,references}/pr-body.md` に委譲。本スキルは「いつ送信するか (= 各ステップ完了コミット直後)」と「どこに保持するか (= GitHub のみ、リポジトリ非永続)」を規定し、テンプレートの中身と更新ガイドはそちらで集約する
