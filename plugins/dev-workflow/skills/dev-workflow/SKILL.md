...
各ステップは「成果物を作成 → Exit Criteria 検証 → Gate 通過 → 次ステップ」の流れ。フェーズという中間階層は持たない。
...
| 4    | Task Decomposition   | `planner` × 1                  | User | `task-plan.md`                                   | `specialist-planner`              |
| 5    | Implementation       | `implementer` × N (タスク並列) | Main | コード diff + `TODO.md`                          | `specialist-implementer`          |
| 6    | Self-Review          | `self-reviewer` × 1            | Main | `self-review-report.md`                          | `specialist-self-reviewer`        |
| 7    | External Review      | `reviewer` × N (観点並列)      | User | `review/<aspect>.md`                             | `specialist-reviewer`             |
...
### Step 5 着手前: タスクリスト反映 (必須手順)

`planner` が Step 4 で生成した `task-plan.md` は**不変な計画書**として保持し、Step 5 以降で追跡するタスク状態は **2 箇所**に反映する。
...
セッション内部リストは揮発性のため、**作業引き継ぎ・中断再開のために `docs/dev-workflow/<identifier>/TODO.md` に永続化**。Step 5 開始時に `task-plan.md` から生成し、タスク完了のたびに更新・コミット。

**フォーマット詳細:** `shared-artifacts/references/todo.md` + `shared-artifacts/templates/TODO.md`。

**Step 5 における運用:**

- Step 5 開始時: `task-plan.md` から TODO.md を生成 (全タスク `status: pending`) してコミット
...
**前提:** Step 5 開始時に `TODO.md` と内部タスクリスト (TaskCreate) が `task-plan.md` から反映されていること (前項参照)。
...
- **タスクごとに分割された全コミット**がリポジトリに存在する (Step 5 = 複数コミット、タスク単位)
...
- 特定タスクの `implementer` が期待外の成果物 → **当該タスク担当の `implementer` インスタンス**に指摘を差し戻して再試行 (Step 5 完了まで同一インスタンス維持)。TODO.md は `in_progress` のまま保持
- タスク数が不足 → `implementer` を**追加並列起動** (既存は維持)。**`task-plan.md` は Step 5 中を通じて不変**のため、新規タスクは TODO.md の「後発追加タスク」セクションにのみ追記する。差分の理由も TODO.md 冒頭に記録。多発する場合は Step 4 へのロールバック検討
- タスク定義が不適切 → Step 4 にロールバック (Step 5 を抜ける時点で全 `implementer` 役割終了。再入後は新規 `implementer` を起動)。再生成された `task-plan.md` から TODO.md を**再構築** (既存 TODO.md は `TODO.md.pre-rollback-<timestamp>` にリネーム)
...
1. `self-reviewer` を起動し、全 diff (Step 5 で生成) / `design.md` / Intent Spec / Task Plan を渡す
...
5. 最終的に High 0 件を確認して Exit Criteria 判定 (Step 6 の `self-reviewer` は Exit Criteria 確定まで維持)
...
**Gate:** Main 判定。High 指摘残があれば Step 5 に戻る (Step 7 へは進めない)
...
High 指摘があった場合のループ構造。Step 5 と Step 6 は別ステップなので、ステップを跨ぐ Specialist 再利用は行わないが、**各ステップの活性化期間中はインスタンスを維持**する。
...
2. 観点ごとに `reviewer` を **並列起動** (Step 5 / 6 の `implementer` / `self-reviewer` とは**別個の新規インスタンス**)
...
- Blocker 指摘発生 → Step 5 に戻り修正 (再活性化時は新規 `implementer` 起動)。戻り後は Step 6 → Step 7 を再実行 (Step 7 は新規 `reviewer` を起動)
...
1. `retrospective-writer` に全ステップの進捗記録 / 全成果物 (Intent Spec / Research Notes / Design Document / Task Plan / diff / 各種 Report / 起票したプロジェクト横断 ADR があれば) / Blocker 履歴 / ループ回数 (Step 5 ↔ Step 6 の往復、Step 7 の差し戻し回数) / ユーザー承認ゲートでのやり取りを渡す
...
4. **Step 5 を再開する場合**: `TODO.md` を読み込み、`TaskCreate` で内部タスクリストを**完全復元**する
...
- ステップ完了時のコミット規約 (1 ステップ = 1 コミット、Step 5 のみタスク単位)
...
- **1 ステップ = 1 コミット** (Step 5 を除く)
- **Step 5 のみタスク単位で複数コミット** (task-plan の粒度に従う)
...
| 5. Implementation       | **タスクごとに 1 コミット** (実装 diff + `TODO.md` 該当タスク状態 + `progress.yaml`、必要に応じ `implementation-logs/<task-id>.md`) |
| 6. Self-Review          | `self-review-report.md` + `progress.yaml`                                                                                           |
...
| 9. Retrospective        | `retrospective.md` + `progress.yaml` (最終コミット、`status: completed`)                                                            |
...
feat(dev-workflow/<identifier>/task-<id>): <task summary>   # Step 5 Implementation
...
| 4. Task Decomposition   | 低             | 全体俯瞰が必要なので 1 名                |
| 5. Implementation       | 高             | Task Plan の独立タスクごと               |
| 6. Self-Review          | 低             | 全体整合性が必要なので原則 1 名          |
| 7. External Review      | 高             | レビュー観点ごと (セキュリティ / 性能等) |
...
| Step 4       | 依存関係が解決不能     | Step 3         |
| Step 5       | タスク定義が不適切     | Step 4         |
| Step 5       | 設計との整合性が崩れる | Step 3         |
| Step 6       | High 指摘              | Step 5         |
| Step 6       | 設計レベルの問題       | Step 3         |
| Step 7       | Blocker 指摘           | Step 5         |
| Step 8       | 実装バグ               | Step 5         |
| Step 8       | 設計ミス               | Step 3         |
