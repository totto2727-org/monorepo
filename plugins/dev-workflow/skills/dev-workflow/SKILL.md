...
6. **サイクル内で完結しない判断 (現サイクルを跨いで影響する判断)** が発生した場合のみ、`adr` スキルを使って別途 ADR を起票 (起票モードは `adr/SKILL.md` の「モード判定フロー」に従う。後述「ADR の起票条件」も参照)
...
`design.md` とは別に ADR を起票するのは、**現サイクル内で完結せず、他のサイクル / 他のロードマップ / プロジェクト全体に影響が及ぶ意思決定**に限る。`adr` スキルは保存先の異なる 2 モード (General / Roadmap) を持ち、Main が起票時にモードを判定する (詳細: `adr/SKILL.md` の「モード判定フロー」)。
...
- **サイクル外成果物** (ADR (General / Roadmap mode)、一時レポート): `shared-artifacts` SKILL.md の「サイクル外の成果物」
...
- 現サイクルを跨いで影響する意思決定 (複数 roadmap / 独立した複数 dev-workflow サイクル / プロジェクト全体に及ぶもの、または単一 roadmap 配下の複数サイクルが共有する規範) の記録 → `adr` スキルに委譲 (General / Roadmap mode の使い分けは `adr/SKILL.md` の「モード判定フロー」参照。Design Document の代替にはならない)
