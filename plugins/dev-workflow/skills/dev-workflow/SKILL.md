...
サイクル進行に応じて段階的に埋まる構造。テンプレート本体は `shared-artifacts/templates/pr-body.md`、書き方ガイドは `shared-artifacts/references/pr-body.md` に切り出している。Main は揮発ファイル `$TMPDIR/dev-workflow/<identifier>-pr-body.md` にコピーして埋め、`gh pr edit --body-file` で送信する (リポジトリ内には永続化しない)。
