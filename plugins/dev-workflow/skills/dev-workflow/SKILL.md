...
**reviewer 成果物のファイル運用 (重要):**

review レポートは**観点ごとに 1 ファイル** (`review/<aspect>.md`) で保持する。**Round の有無に関わらず、最終的に「何が指摘され、何を修正し、何を受容したか」が一覧で読み取れる構造**を最優先する (Round 構造で時系列に分割すると一覧性が損なわれるため、Round はメタデータとして列に記録するのみで章節の主軸にしない)。

各ファイルの構造:

1. **メタヘッダー**: 観点 / 検出開始日 / 最終更新日 / 現時点 Gate 判定 (`approved` / `needs_fix`) / 検出された Round 数
2. **指摘一覧テーブル** (一覧性の中核): 1 行 = 1 指摘。列構成は最低以下を含む:
   - `ID` (例: `M-1`, `m-1`、Major / Minor / Info)
   - `深刻度` (`Blocker` / `Major` / `Minor` / `Info`)
   - `指摘内容` (1〜2 文の要約)
   - `状態` (以下のラベルから 1 つ):
     - **`fixed`**: 修正済 (commit SHA を必ず併記)
     - **`partial`**: 一部解消 (残課題を Notes 列に記述)
     - **`pending`**: 未解消で次 Round で対応予定 (担当タスク ID)
     - **`accepted-as-is`** (= 「無視 / 受容」): ユーザー承認で Retrospective 繰越合意 / 対応不要と確定 (合意日付)
     - **`obsolete`**: 設計変更等で無効化 (理由)
   - `検出 Round` (1, 2, ... — 同じ指摘が複数 Round で再検出された場合は最初の Round のみ記録)
   - `解消 Round / 解消 commit` (該当する場合)
   - `Notes` (補足、対応コミットメッセージ抜粋、無効化理由等)
3. **詳細セクション** (任意、必要な指摘のみ): テーブル要約だけでは伝わらない指摘について、根拠コード抜粋・推奨アクションの詳細を `### M-1 詳細` のような形で個別記述。テーブルから見出しに `[詳細](#m-1-詳細)` 等のリンクを貼る
4. **Round 履歴メタ** (任意、ファイル末尾): 各 Round の実行日 / 起動 reviewer 名 (= インスタンス ID) / その Round 単独の Gate 判定を表形式で残す。本セクションは監査用で、本筋の一覧読解には影響させない

これにより、ファイルを開いた読者は**最初に指摘一覧テーブルを見るだけで「全指摘の最終状態」を把握できる**。Round の進行履歴は付帯情報として末尾に追いやり、一覧性を損なわせない。

別ファイル (例: `review/round2-<aspect>.md`) は作らない: 「Round 1 はファイル A、Round 2 はファイル B」だと読者が両方を見比べる負担が発生するため。同一ファイル内で重複指摘を統合し、最新状態のみをテーブルに残すのが基本。
