...
- **Fresh PR + CI before any user-facing checkpoint** — User gates, In-Progress user inquiries, optional artifact previews, and information-share readouts are all preceded by regenerating the PR description (`share-pr-manager` §2) and confirming the matching CI run is PASS (`share-ci-monitoring` §2-3). The user is never exposed to a stale PR view or to an artifact whose CI state is unverified. See the "PR / CI integration trigger table" below for the full trigger row.
...
| **Before any user-facing checkpoint** (User gate, In-Progress user inquiry, optional artifact preview, information-share readout) | (1) Regenerate the PR description so it reflects the latest cycle commit. (2) Confirm the matching CI run is PASS via the double-check. The user must never be exposed to a stale PR view or to an artifact whose CI is unverified. | `share-pr-manager` §2 + `share-ci-monitoring` §2-3                                                  |
...
- **User-facing checkpoints require fresh PR + CI** — every User gate, every In-Progress user inquiry, every optional artifact preview, and every information-share readout is preceded by (a) regenerating the PR description from the latest commit via `share-pr-manager` §2 and (b) confirming the matching CI run is PASS via `share-ci-monitoring` §2-3. This is non-optional even for steps whose gate is Main judgment but which still expose an artifact to the user (e.g. Task Decomposition's optional preview, Step 9 / roadmap Step 4 information shares). The only legitimate skip is when the inquiry's subject is the CI failure itself or the user has been told that no commit has yet occurred.
