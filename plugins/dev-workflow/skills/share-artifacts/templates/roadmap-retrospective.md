...
This document is the **retrospective for the roadmap as a whole**, drafted by the `roadmap-retrospective-writer` Specialist during `dev-roadmap` **Step 4 (Roadmap Retrospective)**. It aggregates each underlying `dev-workflow` cycle's `retrospective.md` and adds roadmap-specific themes (a summary of milestone completion / soundness of the dependency graph / improvement proposals specific to roadmaps). Place it at `docs/retrospective/roadmap-<roadmap-id>.md` (aggregation form, with the `roadmap-` prefix). See `share-artifacts/references/roadmap-retrospective.md` for authoring details.
...
List the final state of `roadmap-progress.yaml.milestones[]` and describe the rationale for each milestone's completion / non-completion / blocked / cancelled judgment.

| Milestone ID       | Title                 | Final Status           | Rationale for the completion judgment | Related dev-workflow `<identifier>`  |
| ------------------ | --------------------- | ---------------------- | ------------------------------------- | ------------------------------------ |
...
## Aggregation of underlying dev-workflow cycles

{{workflow_aggregation}}

Summarize each underlying `dev-workflow` cycle's `docs/retrospective/<identifier>.md` in **one paragraph per cycle**. Each paragraph must include:
...
Improvement proposals for the roadmap layer (the `dev-roadmap` skill / the `roadmap-progress.yaml` schema / milestone decomposition granularity, etc.). Improvement proposals that are self-contained inside an underlying cycle are already written in that cycle's `retrospective.md`, so this section contains only the strategic-level improvements derived from aggregating those.

### Proposed `roadmap-progress.yaml` schema extensions
...
In the current version, "(b) reflecting per-step progress summaries" was scoped out of `roadmap-progress.yaml`. After running real workloads, evaluate whether this policy was sound or whether an extension is needed. If needed, draft a schema extension proposal (`events` array / `last_step` field, etc.) for the next roadmap.

### Retrospective on the `dev-roadmap` ↔ `dev-workflow` integration protocol
...
- Number of underlying `dev-workflow` cycles: {{workflow_count}}
