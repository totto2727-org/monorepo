...
Permanently record decisions whose impact extends **beyond a single dev-workflow cycle** so that future cycles, future roadmaps, and other contributors can rely on the same agreed premises. ADRs complement two volatile artifacts: `design.md` carries cycle-local design decisions; `retrospective.md` / `roadmap-retrospective.md` carry self-evaluation. ADRs are the **long-lived decision layer** that survives both.
...
- **Step where ADRs typically originate:**
  - `dev-workflow` Step 3 (Design) — most common origin
  - `dev-workflow` Step 9 (Retrospective) — when an insight worth permanent recording surfaces
  - `dev-roadmap` Step 1-4 — when a roadmap-shared norm or premise is identified
...
| **Roadmap mode** | `docs/roadmap/<roadmap-id>/adr/<YYYY-MM-DD-title>.md` | Decision is shared by multiple `dev-workflow` cycles under a single roadmap                 |
