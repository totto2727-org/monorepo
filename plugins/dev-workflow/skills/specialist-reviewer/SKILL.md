...
**深刻度ラベル / 状態ラベル / レポート書式:** 詳細仕様は **`share-artifacts/references/review-report.md`** を真のソースとする。本ファイルには重複させない。
...
2. 全 diff を担当観点で通読:
   - 問題が潜みそうな箇所を重点的に検査
   - 必要なら既存コード・類似実装と比較
3. 指摘事項を**深刻度別 (Blocker / Major / Minor / Info)** に分類 (各深刻度の判定基準は `share-artifacts/references/review-report.md` の「深刻度の判定基準」を参照)
4. 各指摘に以下を付記:
   - 該当コミット SHA + ファイル + 行番号
   - 問題の要約と根拠
   - 推奨アクション
...
7. **`share-artifacts/templates/review-report.md` をテンプレートとして** `review/<aspect>.md` を作成 (指摘一覧テーブル形式、状態ラベル付き、Round 履歴メタは末尾に置く構造)
8. Main に提出
