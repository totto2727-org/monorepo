...
`task-plan.md` 不変運用の原則に従い、Construction 中に発見された追加タスクはここに記録する。追加理由を明記すること。
...
- コミットメッセージ例:
  - `docs(ai-dlc/{{identifier}}): start task T1`
  - `docs(ai-dlc/{{identifier}}): complete task T1`
  - `docs(ai-dlc/{{identifier}}): re-activate task T1 (self-review feedback)`
