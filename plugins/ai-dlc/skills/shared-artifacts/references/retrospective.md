...
- **作成ステップ:** Verification Step 9 (Retrospective)
...
| ステップ間ループ                | 回数  | 根本原因     |
| ------------------------------- | ----- | ------------ |
| Step 5 ↔ Step 6                 | {{n}} | {{原因分析}} |
| Construction → Inception Step 3 | {{n}} | {{原因}}     |
...
AI-DLC プラグインのスキル（`main-*` / `specialist-*` / `shared-artifacts`）への具体的な改善提案。
...
`$TMPDIR/ai-dlc/*.md` 一時レポートの件数と主要トピック。件数が多ければ Intent Spec 段階の明確化不足を示唆している可能性あり。
