...
- **作成者:** Main
- **初期化:** サイクル開始時（`inception` Step 1 に着手する直前）
- **更新:** 各ステップ完了時 / ユーザー承認時 / Blocker 発生時 / ロールバック時 / セッション再開時

## ファイル位置

`docs/ai-dlc/<identifier>/progress.yaml`
...
- `phase`: `Inception` / `Construction` / `Verification` / `Completed` のいずれか
...
- `TODO.md` と連携（Construction 中のタスク状態は TODO.md が真のソース、YAML 側はサイクルレベルのサマリ）
