...
implementer チームが生成した**全 diff を統合的にレビュー**し、外部レビュー前に明らかな問題（`design.md` 違反、Intent Spec 未達見込み、明白な bug、テスト網羅不足等）を潰す。Verification Step 7 の外部観点レビューとは別層。

## 作成者 / 作成タイミング

- **作成者:** `self-reviewer` Specialist（単一インスタンス）
- **作成ステップ:** Construction Step 6 (Self-Review)
- **承認:** Main 判定。High 指摘残があれば Step 5 に戻る
...
- Round 1: High 3 件検出、Step 5 に差し戻し
- Round 2: High 0 件、Gate 通過

## Self-Review の焦点

外部観点（security / performance / readability 等）は **Step 7 `specialist-reviewer`** の領域。Self-Review の焦点は以下:
...
- **後続:** `retrospective.md` が High 指摘の根本原因を分析（設計レベルなら Inception 回帰を提案）
