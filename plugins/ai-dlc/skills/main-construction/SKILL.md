...
**プロジェクト固有ルールの適用:** 実装パターン・テスト基盤・命名規則・コミット規約はプロジェクト固有のスキル（`effect-layer`, `effect-hono`, `effect-runtime`, `totto2727-fp`, `git-workflow`, `macos-cli-rules` 等）を優先。Main は `implementer` 起動時に該当スキルのパスを入力に含める。矛盾時は `main-workflow` の「プロジェクト固有ルールとの関係」に従う。
...
- タスク数が不足していたと判明 → `implementer` を**追加並列起動**（既存インスタンスは維持）。**`task-plan.md` は Construction 中を通じて不変**のため、新規タスクは TODO.md の「後発追加タスク（`task-plan.md` 以降に発生したもの）」セクションにのみ追記する（`task-plan.md` 本体は書き換えない）。差分の理由も TODO.md 冒頭に記録。追加タスクが多発する場合は Inception Step 4 へのロールバックを検討
...
**プロジェクト固有ルールの適用:** レビュー観点のうちプロジェクト固有の基準（セキュリティ基準値、パフォーマンス許容値、設計規約違反の定義）はプロジェクト固有スキルに従う。Main は `self-reviewer` 起動時に該当スキルのパスを入力に含める。矛盾時は `main-workflow` の「プロジェクト固有ルールとの関係」に従う。
