...
name: construction
description: >
  AI-DLC Phase 2 (Construction) の詳細実行手順を定義する。
  ...
  Do NOT use for: ai-dlc ワークフロー全体の管理（workflow スキルを使う）、
  意図明確化・設計・タスク分解（inception スキルを使う）、
  外部レビュー・検証（verification スキルを使う）、
  Task Plan がない状態での実装開始。
---
...
**このスキルは `workflow` スキル配下で使用される。** `workflow` の基本方針・役割定義・調整プロトコル・ユーザー承認ゲート規則（Artifact-as-Gate-Review / Report-Based Confirmation for In-Progress Questions の区別）をすべて継承する。
...
**コミット方針:** 実装 diff はタスクごと・コミット単位で分割。`self-review-report.md` と `progress.yaml` は別コミットで追加（詳細は workflow スキルの「成果物保存構造」参照）。

**対応テンプレート:**

- `TODO.md` → `skills/construction/templates/TODO.md`
- `self-review-report.md` → `skills/construction/templates/self-review-report.md`
- `implementation-logs/<task-id>.md` → `skills/construction/templates/implementation-log.md`

Specialist 起動時に該当テンプレートパスを入力に含めること。
...
いずれか欠けていれば Construction に入らず `inception` スキルへ戻る。
...
Source: `task-plan.md`. Maintained during Construction.
...
- [x] **T1** — Setup database schema
  ...
  - commit: abc1234
  - implementer: impl-1
- [x] **T2** — Add user entity
  - status: completed
  - completed_at: 2026-04-24T10:15:00Z
  - commit: def4567
  - implementer: impl-2
...
- `implementer` 起動時: 該当タスクを `status: in_progress` に更新、`started_at` と `implementer` を記録してコミット
- タスク完了時: 該当タスクを `[x]` に変更、`status: completed`、`completed_at`、`commit` SHA を記録してコミット
- タスク再活性化時（Self-Review High 指摘で Step 5 に戻った場合等）: 該当タスクを `status: in_progress` に戻し、`re_activations` 回数を追加して記録

**コミット単位:**

- TODO.md は各タスク状態変化ごとにコミット（頻繁）
- コミットメッセージ例: `docs(ai-dlc/<identifier>): complete task T1`
- 進捗が細かくコミットされることで、中断後の復旧精度が上がる

### 3. TaskCreate と TODO.md の同期ルール

**原則:** TODO.md が**真のソース**（永続層）、TaskCreate はその**ビュー**（揮発層）。セッション跨ぎでは TODO.md から TaskCreate を復元する。

- 状態変化時: **TODO.md 更新 → コミット → TaskUpdate** の順で実行する（永続側を先に確定）
- セッション再開時: `TODO.md` を読み込み、`TaskCreate` で再構築する
- 齟齬が発生した場合: TODO.md を正として TaskCreate を修正
...
- `TODO.md` 内の全タスクが `[x] completed` 状態でコミット済み
- 型チェック・リント・既存テストが通過
...
全項目チェック後、`verification` スキルへ移行する。
...
- ワークフロー全体の管理 → `workflow` スキル
- 意図明確化・設計・タスク分解 → `inception` スキル
- 外部レビュー・検証・振り返り → `verification` スキル
