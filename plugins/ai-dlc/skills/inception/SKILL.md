...
name: inception
description: >
  AI-DLC Phase 1 (Inception) の詳細実行手順を定義する。
  ...
  実装・レビュー・検証フェーズ（construction / verification スキルを使う）、
  フェーズを跨いだ意思決定。
---
...
**コミット方針:** 各ステップ完了時に成果物と `progress.yaml` を追加／更新してコミット（詳細は workflow スキルの「成果物保存構造」参照）。
...
- `design.md` が作成済み・コミット済み
- 主要な設計判断について代替案と採用理由が記録されている
- ユーザーが設計方針に同意済み
- プロジェクト横断 ADR が必要な決定があった場合は、該当 ADR も起票済み・リンク記載済み
...
全項目チェック後、`construction` スキルへ移行する。
