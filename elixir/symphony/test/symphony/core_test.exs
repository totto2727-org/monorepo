...
defmodule Symphony.CoreTest do
  ...
  test "Symphony.start_link delegates to the orchestrator" do
    ...
    if is_pid(orchestrator_pid) do
      assert :ok =
               Supervisor.terminate_child(Symphony.Supervisor, Symphony.Orchestrator)
    end
  ...
  test "non-active issue state stops running agent without cleaning workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-nonactive-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "terminal issue state stops running agent and cleans workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-terminal-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "missing running issues stop active agents without cleaning the workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-missing-running-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "prompt builder reports workflow load failures separately from template parse errors" do
    ...
    assert :ok =
             Supervisor.terminate_child(Symphony.Supervisor, Symphony.WorkflowStore)
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "run the installed `land` skill"
  ...
  test "agent runner keeps workspace after successful OpenCode run" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-agent-runner-retain-workspace-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner forwards timestamped OpenCode updates to recipient" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-agent-runner-updates-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner surfaces ssh startup failures instead of silently hopping hosts" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-agent-runner-single-host-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner continues with a follow-up turn while the issue remains active" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-agent-runner-continuation-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner stops continuing once agent.max_turns is reached" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-agent-runner-max-turns-#{System.unique_integer([:positive])}"
      )
