...
defmodule Symphony.WorkspaceAndConfigTest do
  ...
  test "workspace bootstrap can be implemented in after_create hook" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-hook-bootstrap-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace path is deterministic per issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-deterministic-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace reuses existing issue directory without deleting local changes" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-reuse-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace replaces stale non-directory paths" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-stale-path-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace rejects symlink escapes under the configured root" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-symlink-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-root-symlink-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove rejects the workspace root itself with a distinct error" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-root-remove-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace surfaces after_create hook failures" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-hook-failure-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace surfaces after_create hook timeouts" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-hook-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace removes all workspaces for a closed issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-issue-workspace-cleanup-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace cleanup handles missing workspace root" do
    missing_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-missing-workspaces-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove returns error information for missing directory" do
    random_path =
      Path.join(
        System.tmp_dir!(),
        "symphony-missing-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace hooks support multiline YAML scripts and run at lifecycle boundaries" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-hooks-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-hooks-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails with large output" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-hooks-large-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook times out" do
    ...
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-workspace-hooks-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "config reads defaults for optional settings" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(), opencode_model: "openai/gpt-5.5")
    assert Config.settings!().opencode.model == "openai/gpt-5.5"
  end
  ...
  test "schema parse normalizes env-backed fallbacks and opencode model" do
    ...
    assert settings.opencode.model == "openai/gpt-5.5"
  ...
  test "workflow prompt is used when building base prompt" do
    workflow_prompt = "Workflow prompt body used as opencode instruction."
  ...
  test "remote workspace lifecycle uses ssh host aliases from worker config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-remote-workspace-#{System.unique_integer([:positive])}"
      )
