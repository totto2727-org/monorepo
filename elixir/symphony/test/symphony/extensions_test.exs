...
defmodule Symphony.ExtensionsTest do
  ...
  setup do
    linear_client_module = Application.get_env(:symphony, :linear_client_module)

    on_exit(fn ->
      if is_nil(linear_client_module) do
        Application.delete_env(:symphony, :linear_client_module)
      else
        Application.put_env(:symphony, :linear_client_module, linear_client_module)
      end
    end)
  ...
  setup do
    endpoint_config = Application.get_env(:symphony, SymphonyWeb.Endpoint, [])

    on_exit(fn ->
      Application.put_env(:symphony, SymphonyWeb.Endpoint, endpoint_config)
    end)
  ...
  test "workflow store reloads changes, keeps last good workflow, and falls back when stopped" do
    ...
    assert :ok = Supervisor.terminate_child(Symphony.Supervisor, WorkflowStore)
    assert {:ok, %{prompt: "Third prompt"}} = WorkflowStore.current()
    assert :ok = WorkflowStore.force_reload()
    assert {:ok, _pid} = Supervisor.restart_child(Symphony.Supervisor, WorkflowStore)
  end
  ...
  test "workflow store start_link and poll callback cover missing-file error paths" do
    ...
    restart_result = Supervisor.restart_child(Symphony.Supervisor, WorkflowStore)
  ...
  test "tracker delegates to memory and linear adapters" do
    ...
    Application.put_env(:symphony, :memory_tracker_issues, [issue, %{id: "ignored"}])
    Application.put_env(:symphony, :memory_tracker_recipient, self())
    write_workflow_file!(Workflow.workflow_file_path(), tracker_kind: "memory")

    assert Config.settings!().tracker.kind == "memory"
    assert Symphony.Tracker.adapter() == Memory
    assert {:ok, [^issue]} = Symphony.Tracker.fetch_candidate_issues()
    assert {:ok, [^issue]} = Symphony.Tracker.fetch_issues_by_states([" in progress ", 42])
    assert {:ok, [^issue]} = Symphony.Tracker.fetch_issue_states_by_ids(["issue-1"])
    assert :ok = Symphony.Tracker.create_comment("issue-1", "comment")
    assert :ok = Symphony.Tracker.update_issue_state("issue-1", "Done")
    ...
    assert Symphony.Tracker.adapter() == Adapter
  end

  test "linear adapter delegates reads and validates mutation responses" do
    Application.put_env(:symphony, :linear_client_module, FakeLinearClient)
  ...
  defp start_test_endpoint(overrides) do
    ...
    Application.put_env(:symphony, SymphonyWeb.Endpoint, endpoint_config)
    start_supervised!({SymphonyWeb.Endpoint, []})
  end
  ...
  defp ensure_workflow_store_running do
    if Process.whereis(WorkflowStore) do
      ...
      case Supervisor.restart_child(Symphony.Supervisor, WorkflowStore) do
        {:ok, _pid} -> :ok
