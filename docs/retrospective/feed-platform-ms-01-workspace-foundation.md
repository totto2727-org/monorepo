...
- **Effect 4.x beta における API 仕様の即時検証**: 本サイクル中に `effect@4.0.0-beta.60` が `ServiceMap.Service` 非対応 (`Context.Service` のみ) であることが implementer-A の段階で発見された。design 段階で `node_modules/.bun/effect@.../dist/*.d.ts` を確認する習慣を skill 化する候補
