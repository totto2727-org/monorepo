...
このドキュメントは `dev-roadmap` の **Step 2 (Milestone Decomposition)** で `roadmap-planner` Specialist が起草する**1 マイルストーンの定義書**。
...
- Passkey 実装は WebAuthn 標準準拠とし、ライブラリ選定は配下 `dev-workflow` サイクル Step 2 (Research) で確定
