...
- Current implementation notes are tracked in [`tmp/ms-02-auth-current-design.md`](../tmp/ms-02-auth-current-design.md). New code should treat `js/package/auth/` as the shared auth package: shared setup/require middleware owns Better Auth plumbing, apps provide only unauthenticated policy, app users expose `id`, and `sub` stays at JWT/OIDC payload boundaries.
