...
具体形式: `src/<entry>/worker.ts` + `src/<entry>/wrangler.jsonc` のペアを entry ごとに配置。`wrangler deploy --config src/<entry>/wrangler.jsonc` を entry ごとに実行することで個別デプロイが成立する。ms-01 では `src/health/` + `src/bff/` の **2 entry を雛形として配置** (intent-spec SC-5 = `find` で `worker.ts` ≥ 2 件)。
...
- **バックエンド (`feed-platform-backend`)**: **wrangler 直接実行** (`wrangler deploy --config src/<entry>/wrangler.jsonc`)。Vite ビルドを介さず、Cloudflare Workers ランタイムに直接デプロイ。`vite.config.ts` の `build` task は **no-op** (`command: ''` + `dependsOn: ['setup']`) として定義し、`vp run -r build` の workspace-wide 完走は `wrangler types` で `worker-configuration.d.ts` が生成される `setup` フェーズが担う
...
### D-7: backend multi-entry 規約 = `src/<entry>/worker.ts` + `src/<entry>/wrangler.jsonc` ペア

バックエンドの multi-entry 構造は以下の規約に従う (本 ms-01 の design.md A-1 / A-5 / M-1〜M-4 で確定):

- **ディレクトリ**: `src/<entry>/` 直下に `worker.ts` と `wrangler.jsonc` を配置
...
- **ms-01 で配置する 2 entry**: `src/health/` (`/health` 200 OK) + `src/bff/` (将来のリソースサーバ予告)
...
- **3 プロジェクトの最小雛形が `js/app/` 直下に配置**:
  - `js/app/feed-platform-backend/` (`src/feature/{env,greeting,health,runtime/server,runtime/hono}.ts` + `src/{health,bff}/{worker.ts,wrangler.jsonc}` + `src/smoke.test.ts`)
...
- **backend multi-entry 規約 (`src/<entry>/worker.ts` + `wrangler.jsonc`、`name: feed-platform-backend-<entry>`)** は ms-02 以降でも維持される。新 entry 追加手順は design.md M-1 に従う
