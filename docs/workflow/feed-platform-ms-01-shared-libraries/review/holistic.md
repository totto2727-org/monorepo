...
| m-1 | Minor    | design.md L212-213 (`remix-helper` exports) は `"exports": { ".": "./src/index.ts" }` + barreled `src/index.ts` を規定するが、実装は barreled file を置かず `"exports": { ".": "./src/frame-helpers.ts" }` の直接 export に変更。effect-hono も同様に barrel `src/index.ts` なしで subpath exports に変更。oxc no-barrel-file rule 回避が理由。                                                                                                                                             | accepted-as-is      | c7581bb, 9869621  | design.md L108 (`src/index.ts (re-export barrel)`) と L213 (`src/index.ts`) の 2 箇所が実装と不一致。design.md を subpath exports に訂正すべきだが、本 cycle 内で訂正が急務ではない (= 実装が正、API surface の semantics は維持)。Step 9 retrospective で design.md の即時修正 protocol 候補として記録。                                                                                                                                                                                                                          |
...
### m-1 detail: barrel index.ts の elimination (subpath exports 化)

**design.md 規定 vs 実装:**

| 規定箇所                | design.md                                                    | 実装                                                                        |
| ----------------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------- |
| L107-108 (effect-hono)  | `exports: { ".": "./src/index.ts" }` + `src/index.ts` barrel | `exports: { "./env": ..., "./logger": ..., "./runtime": ... }`、barrel なし |
| L212-213 (remix-helper) | `exports: { ".": "./src/index.ts" }` + `src/index.ts` barrel | `exports: { ".": "./src/frame-helpers.ts" }`、barrel なし                   |

**理由:** oxc no-barrel-file rule 回避 (review 指示に記載)。subpath exports は barrel より推奨されるモダンなパターンであり、API surface の semantics は不変。

**Severity 判定:** Minor。実装が正しく、API surface の semantics (何が import 可能か) に変更はない。design.md の記述のみが古い。Step 9 retrospective で design.md 即時修正 protocol 候補として記録。

**effect-hono の subpath exports (commit `c7581bb`):**
...
Consumer 側の import 文もこれに対応:

- `import { Env } from 'effect-hono'` → `import { Service, layer, makeLayer } from 'effect-hono/env'`
