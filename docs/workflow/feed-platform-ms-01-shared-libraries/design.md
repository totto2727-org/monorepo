...
- **U-other-D**: frame name 型表現について複数の経緯を経て確定。当初 Pattern P2 (`<const T extends readonly string[]>` + `T[number]`) → User 指摘 2026-05-09 で撤回 → Record 形 (`Record<PropertyKey, string>` + `T[keyof T]`) → User 再指摘 2026-05-09 で撤回 → **`<T extends string>` (string literal union 直接受け)** に最終確定。型関数 `InferFrameName<T>` は不要 (= 渡された `T` 自体が frame name union)。consumer は `type Frame = 'content' \| 'sidebar'` のように union 型を直接定義し、`createFrameHelpers<Frame>()` でジェネリクス引数に渡す形。
...
graph TB
  subgraph LIB["js/package/* (新規 2 library)"]
    ...
    RH["remix-helper<br/>(C-4+5+6 createFrameHelpers + helpers.FrameLink)"]
  end
...
U-3 + U-other-D 採用により、各 consumer project の `routes.ts` (またはそれに相当する場所) は以下の helper を持つ:
...
これにより `remix-helper` 側は `hono` への直接依存がなく (= `peerDependencies` に `hono` を持たない)、consumer 側は **string literal union 1 行宣言 + helpers 取り出し** だけで従来の `isFrameRequest` / `FrameLink` API を維持できる。`frames` 配列値や `Record` オブジェクトを介さず、union 型を直接 generics 引数に渡す最もシンプルな形 (User 確定 2026-05-09)。
...
User 指摘 (2026-05-09) を受けて Phase 1 の struct 形 (`{ ENV: ... }`) を **直接 string literal union 型** に simplify する。`yield* Env.Service` の戻り値が `'production' | 'development'` 文字列直接になり、consumer 側の `env.ENV` アクセスが `env` 直接アクセスに置き換わる。
...
User 指摘 (2026-05-09): `Layer.unwrap` / `Layer.provide(Env.layer)` を library 側で行うのは coding 規約違反 (= unwrap 含め依存を解決して良いのは原則アプリケーションの依存の大元 = consumer entry point だけ)。library 側は **Env を依存として要求する Layer** をそのまま export し、Env 依存の closure は consumer 側 entry point に委ねる。

```typescript
import { Layer, Logger } from 'effect'
import * as Env from './env.ts'

// Env Service から `ENV` 値を取得して Logger 形式 (consoleJson / consolePretty) を選択する Layer。
// 内部で `Env.Service` を Effect の context から読み出し、その値に応じて Logger を切り替える。
// 戻り値は Env を依存として要求する `Layer<never, never, Env.Type>` (= Env-open)。
// `Layer.unwrap` / `Layer.provide(Env.layer)` は consumer 側 entry point の責務 (本 design.md
// "Consumer migration の adapter 統一 pattern" 参照)。
export const dynamicLoggerLayer: Layer.Layer<never, never, Env.Type> = /* Effect 4.x API で実装 */
```

**Step 6 implementer note (具体的 API 確定):** `Layer.unwrap` を使わず Service 値に応じて Logger を切り替える Layer を構成する Effect 4.x API は、`/Users/totto2727/.warp/worktrees/monorepo/roadrunner-pinyon/node_modules/effect/dist/dts/Logger.d.ts` および `Layer.d.ts` を引いて確定する。候補は以下:

- `Logger.replaceEffect(Logger.defaultLogger, effect)` 形 (Effect から Logger を生成して default を置き換える Layer を作る API があれば)
- `Layer.effect(LoggerService, effect)` 形 (Logger Service tag を直接受ける Layer.effect が利用可能なら)

いずれの形でも、内部の Effect は `yield* Env.Service` で `Env.Type` を要求するため、戻り値の Layer は `Layer<..., ..., Env.Type>` として自然に Env-open になる。Phase 1 の `Layer.unwrap` 形は撤回。

**library 提供:** Env を依存として要求する `Layer<never, never, Env.Type>` (= Env-open)。consumer は `Layer.provide(dynamicLoggerLayer)` 後に **必ず `Env.layer` も合わせて provide して依存を閉じる** 必要がある。

**consumer 提供:** Env 依存を closure するための `Layer.provide(Env.layer)` 連結。Logger 形式判定の条件式 (`env === 'production'`) と Logger 関数 (`consoleJson` / `consolePretty()`) は library 内ハードコード (Intent Spec L64-L66 「ひとまず」「将来 factory 化する余地は残すが本 cycle では未抽象」)。
...
// package.json: { "exports": { ".": "./src/index.ts" } }
// src/index.ts
export * from './frame-helpers.ts' // createFrameHelpers
```

#### `src/frame-helpers.ts` (C-4 + C-5 + C-6, U-3 採用 + Union 直接受け)

User 確定 (2026-05-09): `frames` のランタイム値や registry オブジェクトは渡さず、**string literal union 型を generics 引数で直接受ける** 最もシンプルな形を採用。型関数 `InferFrameName<T>` は不要 (= 渡された `T` 自体が frame name union)。FrameLink (C-6) を新たに移植対象に追加し、`createFrameHelpers` の戻り値プロパティとして提供する。

```typescript
import { createElement, type Handle, type Props, type RemixNode } from 'remix/ui'

// ----- FrameLink Props (rmx-target は wire format value = T 直接) -----
...
interface FrameHelpers<T extends string> {
  ...
  /**
   * Anchor that always navigates a named frame (`hono-remix-v3-cloudflare-example/app/ui/frame-link.tsx`
   * 完全継承)。`rmx-target` prop は型 `T` に拘束され、consumer の frame name union 外の
   * 文字列を渡すと型エラーになる。
   */
  readonly FrameLink: (handle: Handle<FrameLinkProps<T>>) => () => RemixNode
}

// ----- factory (type-only specialization, ランタイム引数なし) -----
...
**library 提供:** `createFrameHelpers<T>()` factory のみ (型関数 `InferFrameName` 廃止)。`FrameLink` は helpers の戻り値プロパティとして提供。**`hono` への直接依存ゼロ** (U-3 完全達成、`peerDependencies` も `hono` 不要)。

**consumer 提供:**

- frame name string literal union 型 (例: `type FrameName = 'content' | 'sidebar'`)
...
- `PageOrFrame` の `Request` bind (例: `const PageOrFrameForRequest = helpers.createPageOrFrame('content', Layout)(getContext().req.raw)`)
- `FrameLink` re-export (`export const FrameLink = helpers.FrameLink`、specialize 済み component を consumer 名前空間で公開)

#### TS 5.9 環境での実機検証 (User 確定 2026-05-09 反映)

- `<T extends string>` ジェネリクス制約は `@tsconfig/strictest` + `target: esnext` 環境で素直に通る (literal を含む union がそのまま `T` に bind される)
- consumer 側は **string literal union を 1 行で型宣言** (`type FrameName = 'content'`) して `createFrameHelpers<FrameName>()` で渡す。`as const` / tuple / Record 等の一切不要
- 既存 Phase 1 実装 (`frames = { content: 'content' } as const` Record 形 / `frames = {} as const` 空 object) は migration で **string literal union 直接定義に書き換え** (`type FrameName = 'content'` / `type FrameName = never` 相当 — 空の場合は frames 不在として扱う)
- Step 6 implementer は `expectTypeOf(helpers.isFrameRequest).parameter(1).toEqualTypeOf<'content'>()` 等で type-level test 可能
- `T extends string` には `never` も含まれるため、空相当 (`type FrameName = never`) でも型エラーにならず `isFrameRequest` の引数型が `never` に縮退する形で安全に扱える
...
├── vite.config.ts  # 不要
└── src/
    ├── index.ts          # barrel: createFrameHelpers re-export
    ├── frame-helpers.ts  # C-4 + C-5 + C-6 統合 (createFrameHelpers factory + helpers.FrameLink)
    └── frame-helpers.test.ts  # SC-3 smoke test (isFrameRequest + FrameLink type-level)
```

**注:** C-6 `FrameLink` は `createFrameHelpers` の戻り値プロパティとして提供するため、別ファイル `frame-link.tsx` には分離せず `frame-helpers.ts` に統合 (factory が単一クロージャで FrameLink まで合成、JSX は `createElement` 経由のため `.ts` 拡張子で OK)。
...
2. **Consumer migration** (atomic 単位 2、中間状態は許容しない):
   ...
   - **`feed-platform-web`**: 上記同形 + `app/routes.ts` を library + union 直接受け pattern に置換 (空相当 `type FrameName = never` で SC-6 達成、option A)。
   - **`identity-provider`**: 上記同形 (web と完全並列実装可)。
   - **`hono-remix-v3-cloudflare-example`**: `app/routes.ts` を `type FrameName = 'content'` + `createFrameHelpers<FrameName>()` の union 直接受けパターンに置換。`app/ui/page-or-frame.tsx` (PageOrFrame) と `app/ui/frame-link.tsx` (FrameLink) は **両方削除** し library helpers に置換 (= C-5 + C-6 完了)。`app/ui/content-layout.tsx` の `createPageOrFrame(frame, Layout)(request)` の `request` bind は consumer adapter で行う必要があり、これは `getContext().req.raw` を渡す 1 行 helper として `routes.ts` に追加。`FrameLink` を import する consumer 側コードは `import { FrameLink } from '../routes.ts'` の path 不変で migration 完了。Counter / TODO 既存 behavior は SC-7 で保持。
3. **削除確認**:
   - `grep -rE 'dynamicLoggerLayer|DisposableRuntime' js/app/{feed-platform-backend,feed-platform-web,identity-provider,hono-remix-v3-cloudflare-example}/` で 0 hit (SC-5 観測仕様、effect-hono 移植分)
   - `grep -rE 'isFrameRequest =|createPageOrFrame =' js/app/{feed-platform-web,identity-provider,hono-remix-v3-cloudflare-example}/` で library 経由以外 0 hit (SC-6 観測仕様、remix-helper 移植分)
   - `ls js/app/hono-remix-v3-cloudflare-example/app/ui/{page-or-frame,frame-link}.tsx` で 2 ファイル不在 (C-5 / C-6 削除確認)
...
- 加えて `hono-remix-v3-cloudflare-example` のみ `app/ui/frame-link.tsx` (約 30 行) + `app/ui/page-or-frame.tsx` (約 30 行) 削除で **追加 60 行純減**
...
| **D**           | frame name 型を Pattern P2 (`readonly string[]` + `T[number]`) または Record (`T[keyof T]`) で抽出  | Rejected           | User 確定 (2026-05-09) で両形式撤回。`createFrameHelpers<T extends string>()` で **string literal union を generics 引数で直接受ける** 最もシンプルな形を採用 (= 型関数 `InferFrameName` 不要、tuple/Record 一切不要)。Adopted version は本表 A 行に統合 |
...
- **`remix-helper` の `createFrameHelpers` 拡張**: 現状 helpers の戻り値は `isFrameRequest` / `createPageOrFrame` / `FrameLink` の 3 メンバ。将来 `<Frame>` (server side fragment renderer) や `useFrameRequest()` (client side hook) 等が必要になった場合、helpers に追加する余地を残す (= consumer 側 import path 不変)。
...
- **`remix-helper/src/frame-helpers.test.ts`**: `createFrameHelpers<'a' | 'b'>()` で生成、`new Request('http://x', { headers: { 'x-remix-target': 'a' } })` を渡して `isFrameRequest` の true / false 動作を確認。`expectTypeOf(helpers.isFrameRequest).parameter(1).toEqualTypeOf<'a' | 'b'>()` で型レベル assertion、`// @ts-expect-error` で `'unknown'` 等の境界外を渡せないことを assert。`helpers.FrameLink` の `rmx-target` prop 型が `'a' | 'b'` に拘束されることも type-level test で確認。
...
| `<T extends string>` での string literal union 直接受け推論 (Research D の P1 / P2 比較は不要、最もシンプルな形)      | Step 6 implementer が library 実装直後に `vp run --filter remix-helper check` で TS error 不在を確認、type-level test (`expectTypeOf`) で `T` がそのまま frame name union として bind されることを assert |
...
| SC-6  | 3 projects (web / IdP / hono-remix-example) で `createFrameHelpers` 利用                  | "Consumer migration の adapter 統一 pattern" 節 (各 project の `routes.ts` で 1 hit、option A 採用)                                       |
...
2. **Consumer migration** (atomic 単位 2、中間状態許容なし):
   ...
   - **T-D**: `feed-platform-web` の同形 + `app/routes.ts` 置換 (`type FrameName = never` で空相当)
   - **T-E**: `identity-provider` の同形 (T-D と完全並列、コピー作業に近い)
   - **T-F**: `hono-remix-v3-cloudflare-example` の `app/routes.ts` を `type FrameName = 'content'` + `createFrameHelpers<FrameName>()` の union 直接受けパターンに置換、`app/ui/page-or-frame.tsx` + `app/ui/frame-link.tsx` 両ファイル削除 (= C-5 + C-6)、`app/ui/content-layout.tsx` の `createPageOrFrame` 呼出 adapter 経由化
...
- **T-F は T-B 完了後に開始可能**: T-C-T-E と並列化 OK だが、`hono-remix-v3-cloudflare-example` のみ frame name union が non-empty (`'content'`) + `frame-link.tsx` 削除を含むため test 設計の難易度が高い (SC-7 観測のため Step 4 QA Design で smoke 仕様確定後に着手推奨)
...
- `grep -rn 'createFrameHelpers' js/app/{feed-platform-web,identity-provider,hono-remix-v3-cloudflare-example}/` で各 ≥ 1 hit (SC-6)
...
- C-7 〜 C-12 の追加 candidate 抽出 — ms-02 以降で個別判断 (C-6 FrameLink は本 cycle で追加採用済)
...
- (なし) — `FrameLink` は当初 scope 外候補だったが本 cycle (User 確定 2026-05-09) で C-6 として正式に library 化対象に追加。Out of scope の Research B I-6 残置候補は本 cycle で解消済
