...
- **U-other-D**: `InferFrameName<T>` は当初 Pattern P2 (`<const T extends readonly string[]>` + `T[number]`) を採用したが、User 指摘 (2026-05-09) により撤回。`frames` の基底型は `Record<PropertyKey, string>` (既存実装 `(typeof frames)[keyof typeof frames]` と完全整合) として、`InferFrameName<T extends Record<PropertyKey, string>> = T[keyof T]` (= P1 寄り、value union) を採用。本 design.md 内で TS 5.9 環境での実機検証可能性を確認 (後述「Public API surface — InferFrameName」)。
...
#### `src/frame-helpers.ts` (C-4 + C-5, U-3 採用 + frames Record ベース)

User 指摘 (2026-05-09): `frames` の基底型は `Record<PropertyKey, string>` (= 既存実装 `(typeof frames)[keyof typeof frames]` と完全整合)。Research D で当初採用した Pattern P2 (`readonly string[]` + `T[number]`) は撤回し、Pattern P1 寄り (Record + `T[keyof T]`) に確定。

```typescript
import { createElement, type Handle, type RemixNode } from 'remix/ui'

// ----- 型関数 (Record ベース + value union) -----

/**
 * 入力 frame names registry から string literal value union を抽出する型関数。
 * Pattern: input `Record<PropertyKey, string>` ジェネリクス + `<const T>` + `T[keyof T]`。
 *
 * @example
 *   const frames = { content: 'content' } as const
 *   const helpers = createFrameHelpers(frames)
 *   type FrameName = InferFrameName<typeof helpers.frames>  // 'content'
 */
export type InferFrameName<T extends Record<PropertyKey, string>> = T[keyof T]

// ----- value 側 factory (U-3 採用 = Hono フリー signature) -----

interface FrameHelpers<const T extends Record<PropertyKey, string>> {
  readonly frames: T
  /**
...
export const createFrameHelpers = <const T extends Record<PropertyKey, string>>(frames: T): FrameHelpers<T> => ({
  frames,
...
- frame names registry (例: `{ content: 'content' } as const`)
...
#### TS 5.9 環境での実機検証 (User 指摘 2026-05-09 反映)

- `<const T extends Record<PropertyKey, string>>` (TS 5.0+ const type parameters) は `@tsconfig/strictest` + `target: esnext` 環境で利用可能 (Research D F-4)
- inline object literal `createFrameHelpers({ content: 'content' })` は **`as const` 不要** (`<const T>` 効果)
- 既存実装 `(typeof frames)[keyof typeof frames]` (3 projects + `hono-remix-v3-cloudflare-example`) と意味論完全整合
...
- `frames = {} as const` (空 object) を渡した場合 `T[keyof T] = never` で `isFrameRequest` が型レベル uncallable に縮退 — Phase 1 既存実装 (web / IdP の `frames = {} as const`) と同じ挙動を継承 (Research B I-5)
...
2. **Consumer migration** (atomic 単位 2、中間状態は許容しない):
   ...
   - **`hono-remix-v3-cloudflare-example`**: `app/routes.ts` を library + adapter pattern に置換 (`{ content: 'content' } as const`)。`app/ui/page-or-frame.tsx` ファイル自体は **削除可能** (library で代替) — ただし `app/ui/content-layout.tsx` の `createPageOrFrame(frames.content, Layout)(request)` の `request` bind は consumer adapter で行う必要があり、これは `getContext().req.raw` を渡す 1 行 helper として `routes.ts` に追加。Counter / TODO 既存 behavior は SC-7 で保持。
...
| **D**           | `InferFrameName<T>` を Pattern P2 (`readonly string[]` + `T[number]`)                               | Rejected           | User 指摘 (2026-05-09) で撤回。`frames` の基底型は `Record<PropertyKey, string>` (既存実装 + Research B I-1/I-2 整合) のため、Record + `T[keyof T]` (= P1 寄り) が正解。Adopted version は本表 A 行に統合 |
...
- **`remix-helper/src/frame-helpers.test.ts`**: `createFrameHelpers({ a: 'a', b: 'b' } as const)` で生成、`new Request('http://x', { headers: { 'x-remix-target': 'a' } })` を渡して `isFrameRequest` の true / false 動作を確認。`InferFrameName<typeof helpers.frames>` の type-level assertion (`expectTypeOf` または `// @ts-expect-error`) で `'a' | 'b'` (value union) 復元を確認。
...
| `<const T extends Record<PropertyKey, string>>` の TS 5.9 実機推論 (Research D I-2 / I-4 を Record 形に読み替え)      | Step 6 implementer が library 実装直後に `vp run --filter remix-helper check` で TS error 不在を確認、type-level test (`expectTypeOf`) で `T[keyof T]` value union 復元を assert |
...
2. **Consumer migration** (atomic 単位 2、中間状態許容なし):
   ...
   - **T-F**: `hono-remix-v3-cloudflare-example` の `app/routes.ts` + `app/ui/page-or-frame.tsx` 削除 + `app/ui/content-layout.tsx` の `createPageOrFrame` 呼出 adapter 経由化 (`{ content: 'content' } as const`)
