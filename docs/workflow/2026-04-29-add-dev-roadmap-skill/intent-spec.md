...
9. **README 更新**: `plugins/dev-workflow/README.md` に `dev-roadmap` スキルの存在と「1 サイクル超の大規模開発を束ねる戦略層」相当の位置づけが 1 段落以上で記述されている (README は英語ベースのため、表記は英語で `strategic layer` / `bundles multiple cycles` / `large-scale` 等の同義表現を許容)
