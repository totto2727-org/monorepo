...
- [x] **T6 (post-Wave-3, pre-Step-7)** — `vp check --fix` で cycle artefacts (design.md / qa-design.md / research/\*.md / task-plan.md / TODO.md) の oxfmt formatting 違反を修正。commit: `d0250d1 style(dev-workflow/2026-05-03-pr-ci-integration): apply oxfmt formatting to cycle artefacts`。完了時刻: 2026-05-03T05:55:00Z。
- [x] **T7 (post-Wave-3, pre-Step-8)** — qa-design.md TC-009 のパターン不整合を修正。`^## (サイクル全体の流れ|各ステップの責務|ステップ完了時のコミット規約)` を実 SKILL.md の見出し `^## (役割定義|ステップ詳細|ステップ完了時のコミット規約)` に補正済 (3 件すべて存続確認、`grep -c` 実測 = 3)。完了時刻: 2026-05-03T06:00:00Z。
...
- [x] **T3** — dev-workflow/SKILL.md コミット規約セクション末尾に PR-CI プロトコルへの参照リンクを追加
  - status: completed
  - dependencies: T1, T2 (同一ファイル編集のため Wave 跨ぎで直列化)
  - started_at: 2026-05-03T05:42:00Z
  - completed_at: 2026-05-03T05:48:00Z
  - commit: 0001ed9
