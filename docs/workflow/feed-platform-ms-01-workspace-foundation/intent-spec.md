...
- **Last updated:** 2026-05-04T13:10:00Z
...
- **配置先: `js/app/feed-platform/` のみ** (Q2.5 確定 — 2026-05-04)
  - 単一アプリパッケージとして開始。子パッケージ分割 (BFF 別 / Worker 別 / 共通 package 切り出し) は後続マイルストーンの設計で必要になり次第実施
  - 根拠: YAGNI 原則。マイクロサービス境界としての分割は「需要が確認できた時点」で実施する方が、雛形段階での過剰抽象化を避けられる
  - ms-01 段階では `js/app/feed-platform/` 単独に最小限のディレクトリ構造を整備し、後続マイルストーン (ms-02 認証 / ms-05 永続化等) が子パッケージ追加 / 共通ライブラリ抽出を判断する

### 未確定 (後続ターンで追記)

- 雛形整備の対象範囲 (Lint / Format / 型チェック / テスト / ビルド設定の境界)
- 横断 ADR の起票範囲 (H1 / H4 / H6 / H7 + 任意 H2 / H3 / H5)
