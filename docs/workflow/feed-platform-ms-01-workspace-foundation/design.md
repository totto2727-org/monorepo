...
export const make = (env: Env.Type) =>
  ManagedRuntime.make(
    Health.layer.pipe(
      ...
      Layer.provide(Logger.layer([import.meta.env.PROD ? Logger.consoleJson : Logger.consolePretty])),
    ),
  )
...
import { make as makeRuntime, type Runtime } from './server.ts'

export type Variables = { readonly runtime: Runtime }
...
sequenceDiagram
  ...
  MW->>MW: makeRuntime(c.env) (per-request)
  ...
  Note over MW: try { next() } finally { await runtime.dispose() }
```
...
- **`Logger.layer` を ms-01 で必須採用**: saas-example が採用済み + Cloudflare Workers の標準 `console.log` だけでは PROD/DEV 区別が薄い。`import.meta.env.PROD` で `consoleJson` / `consolePretty` を切替 (saas-example/src/feature/runtime/server.ts:55-56 踏襲)。
- **per-request runtime (= `await using` middleware)**: Cloudflare bindings (`c.env`) は request ごとに変わるため、module top-level の `ManagedRuntime.make` ではなく per-request パターンを採用 (`effect-cloudflare-hono-integration.md` §I-6)。
...
- **Monitoring / observability:** 全プロジェクトの `wrangler.jsonc` で `observability.enabled: true` + `head_sampling_rate: 1` を採用 (`hono-remix-v3-cloudflare-example/wrangler.jsonc:14` 踏襲)。Effect の `Logger.layer([Logger.consoleJson])` (PROD) / `Logger.consolePretty` (DEV) で Worker 実行時の構造化ログを確保 (saas-example/src/feature/runtime/server.ts:55-56 踏襲)。ms-01 段階では Hello World のみのため実観測は ms-02 以降。
...
| 5. Effect 統合の最小実装 (Service 段階数 / Logger.layer / 純 vs Remix-Worker 差) | CC-7 / CC-8 / Alternatives Option K: **3 段階 (succeed/sync/effect) すべて採用、Logger.layer 必須**   |
