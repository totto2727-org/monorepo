...
└── src/
    ...
    │   ├── env.ts                          # Layer.sync (process.env.NODE_ENV 由来) + makeLayer (test 用)
    │   ├── greeting.ts                     # Layer.sync
    │   ├── health.ts                       # Layer.effect (1 依存: Env)
    │   ├── health.test.ts                  # Vitest (Health.check の Effect.runPromise、検証対象 module の隣に配置)
    │   └── runtime/
...
#### A-7. Tests (SC-3) — `feature/<name>.test.ts` colocated

```typescript
// js/app/feed-platform-backend/src/feature/health.test.ts
...
**設計判断 (Open Question 7 解決 + Step 7 → Step 8 user-gate refinement)**:

- **検証対象 module の隣に `feature/<name>.test.ts` を配置** (Step 7 → Step 8 user-gate refinement で確定): 旧設計はソース直下の `smoke.test.ts` 1 ファイルにすべての検証を集約していたが、test 内容と検証対象 module の対応が直感的に追いづらいため、検証対象ごとに専用 test ファイル (= `feature/health.test.ts` 等) に分割し、検証対象の隣に置く co-location 形に変更。`vite-plus/test` の暗黙設定が `**/*.test.ts` を自動収集するため設定変更は不要。`vitest.config.*` は引き続き新設しない (Intent Spec Q2.12 / `vite-plus-task-system-and-existing-packages.md` §H と整合、設定 1 個 = `vite-plus/test` のデフォルトを 3 プロジェクトで共通利用)。
- **配置パターン**: backend は Health のみで 1 件 (`src/feature/health.test.ts`)、web / IdP は Greeting + Health の 2 件 (`app/feature/greeting.test.ts` + `app/feature/health.test.ts`)。describe ラベルは検証対象 module 名 (= `'Health'` / `'Greeting'`)、import は relative (`./env.ts` / `./greeting.ts` / `./health.ts`) — colocation を活かす形。
- **テスト内容**: Effect Service の最小経路 (Layer 合成 + `runPromise`) を踏む。`Effect.provide(program, layer)` で対象 Service を解決し、Health は `Env` 依存を `Env.makeLayer({ ENV: 'development' })` で満たす (test 用 explicit value 注入)。`hono` を import せず Effect 単体で完結させることで cold start / Worker 環境依存をテストから排除。
...
└── app/
    ...
    │   ├── env.ts                          # Layer.sync (process.env.NODE_ENV 由来) + makeLayer (test 用)
    │   ├── greeting.ts
    │   ├── greeting.test.ts                # Vitest (Greeting.greet、検証対象 module の隣に配置)
    │   ├── health.ts
    │   ├── health.test.ts                  # Vitest (Health.check、検証対象 module の隣に配置)
    │   └── runtime/
    │       ├── server.ts
    │       └── hono.ts
    └── ui/
...
#### B-7. Tests (SC-3) — `feature/<name>.test.ts` colocated × 2 件

`feature/greeting.test.ts` (Greeting Service) と `feature/health.test.ts` (Health Service + Env 注入) の 2 件を `app/feature/` 配下に colocation 配置。describe ラベルは検証対象 module 名 (= `'Greeting'` / `'Health'`)、import は relative (`./greeting.ts` / `./env.ts` / `./health.ts`)。

```typescript
// js/app/feed-platform-web/app/feature/greeting.test.ts
import { Effect } from 'effect'
import { describe, expect, test } from 'vite-plus/test'
...
```typescript
// js/app/feed-platform-web/app/feature/health.test.ts
import { Effect, Layer } from 'effect'
import { describe, expect, test } from 'vite-plus/test'
...
#### C-7. Tests (SC-3) — `feature/<name>.test.ts` colocated × 2 件

`feed-platform-web` (B-7) と同形で `app/feature/greeting.test.ts` + `app/feature/health.test.ts` の 2 件を配置。Greeting test は `Greeting.greet('identity-provider')` で `'Hello, identity-provider'` を期待、Health test は backend / web と同一の Env 注入経路を踏む。describe ラベル / relative import 規約は B-7 と完全同形。
...
- **テストファイル配置: 検証対象 module の隣に `feature/<name>.test.ts`** (Step 7 → Step 8 user-gate refinement で確定、旧設計の `smoke.test.ts` 単一ファイル形から分割):
  - `feed-platform-backend/src/feature/health.test.ts` (Health Service テスト 1 件)
  - `feed-platform-web/app/feature/greeting.test.ts` + `feed-platform-web/app/feature/health.test.ts` (Greeting / Health Service テスト 2 件)
  - `identity-provider/app/feature/greeting.test.ts` + `identity-provider/app/feature/health.test.ts` (Greeting / Health Service テスト 2 件)
  - 合計 5 件 (test 件数は不変、配置のみ変更)
...
- **Performance expectations:** Cloudflare Workers の cold start は通常 < 5ms (Hello World レベル)。Effect の `await using` per-request runtime 生成は 1 回 / リクエストの軽量処理 (saas-example で実証済)。SC-3 の test (`feature/<name>.test.ts` × 計 5 件) は 1 件あたり < 100ms 想定 (Effect.runPromise + Layer 合成のみ)。SC-4 の `vp run -r build` は backend の build 未定義 = Vite+ auto-skip により backend を実行しない (web / IdP の Vite ビルドのみが時間支配)。
...
| SC-3  | `vp run test` exit 0 + 各プロジェクトに test ≥ 1                                                                                                           | A-7 / B-7 / C-7 で各プロジェクトに `feature/<name>.test.ts` を colocation 配置 (`vite-plus/test` 経由)。backend は `feature/health.test.ts` 1 件、web / IdP は `feature/{greeting,health}.test.ts` 2 件、合計 5 件 |
...
1. **共通基盤タスク** (3 プロジェクト共通形を `feed-platform-backend` で先行確立、後続 2 プロジェクトでコピー):
   ...
   - T-E: `feed-platform-backend/src/feature/health.test.ts` 配置 (SC-3、Health.check の Effect.runPromise 検証、検証対象 module の隣に colocation)
...
2. **Web フロント / IdP タスク** (T-A〜T-E 完了後に並列開始可能、お互い独立):
   ...
   - T-H: `feed-platform-web/app/feature/{env,greeting,health,runtime/server,runtime/hono}.ts` 配置 + `app/feature/{greeting,health}.test.ts` (検証対象 module の隣に colocation、計 2 件)
...
- `vp run test` で colocation 配置の `feature/<name>.test.ts` (3 プロジェクト合計 5 件) が PASS するか
...
- `src/smoke.test.ts` は本 path restructure 段階では **不変** (src 直下を維持) — 後続の Step 7 → Step 8 user-gate refinement で `src/feature/health.test.ts` に colocation 移動済 (本 design.md 末尾の "Deviation note: Test placement switched to feature/<name>.test.ts colocation" 参照)
...
- SC-3 (test PASS): test は `Env.makeLayer({ ENV: 'development' })` を使うため**影響なし**。実行で全 5 件 (backend `feature/health.test.ts` 1 件 + web/IdP `feature/{greeting,health}.test.ts` 各 2 件) PASS 確認済 — colocation 配置は別途 Step 7 → Step 8 user-gate refinement で確定 (本 design.md 末尾の "Deviation note: Test placement switched..." 参照)。
...
### Deviation note: Test placement switched to `feature/<name>.test.ts` colocation (Step 7 → Step 8 user-gate refinement, 2026-05-06)

**経緯**: ENV detection refinement と同 user gate 内で、User からの追加要望「smoke.test.ts は全て検証ロジックの近くに .test.ts を作成。`feature/*.test.ts` とするのが適切」に基づき、test ファイル配置を **検証対象 module の隣に colocation** する形に変更した。旧設計はソース直下の `smoke.test.ts` 1 ファイルにすべての検証を集約していたが、test 内容と検証対象 module の対応が直感的に追いづらく、検証対象が増えた際の拡張点も曖昧であった。

**変更内容**:

- `js/app/feed-platform-backend/src/smoke.test.ts` (Health 1 件) → `js/app/feed-platform-backend/src/feature/health.test.ts` (Health 1 件、describe ラベル `'Health'`)
- `js/app/feed-platform-web/app/smoke.test.ts` (Greeting + Health 2 件) → `js/app/feed-platform-web/app/feature/greeting.test.ts` (Greeting 1 件) + `js/app/feed-platform-web/app/feature/health.test.ts` (Health 1 件)
- `js/app/identity-provider/app/smoke.test.ts` (Greeting + Health 2 件) → `js/app/identity-provider/app/feature/greeting.test.ts` + `js/app/identity-provider/app/feature/health.test.ts` (同上)
- 全 test ファイルで relative import (`./env.ts` / `./greeting.ts` / `./health.ts`) を採用 (subpath imports `#@/feature/...` から変更、colocation を活かす形)。
- describe ラベルは検証対象 module 名 (= `'Health'` / `'Greeting'`) に揃え、`'<project> smoke'` のような project 名包含表現は廃止。
- test 件数は不変 (合計 5 件)、PASS / FAIL 振る舞いも不変。

**設計上の利点**:

- **検証対象との物理的近接性**: `feature/health.ts` を読む際に `feature/health.test.ts` が同フォルダに存在するため、振る舞い仕様 (test) と実装 (`.ts`) の対応が一目で追える
- **拡張点の明示**: 後続マイルストーン (ms-02 認証 / ms-05 永続化等) で新規 Service module (例: `feature/auth.ts`) を追加する際、隣に `feature/auth.test.ts` を置く規約が自明
- **vite-plus/test の glob 自動収集との整合**: `**/*.test.ts` で自動収集するため設定変更不要 (`vitest.config.*` 新設は引き続きしない、CC-5 規約は不変)
- **describe ラベルの module 整合**: `describe('Health', ...)` のように module 名そのものを使うことで Vitest 出力からも対応関係が明確

**SC への影響**:

- SC-3 (test PASS): `vp test run js/app` で 17 files / 83 tests PASS 確認済 (本サイクル雛形は 5 件、test 件数および PASS / FAIL は完全不変、配置とラベルのみ変更)
- SC-2 (`vp check` exit 0): 確認済、本サイクル雛形のエラーゼロ (rss-graphql の既存エラーは許容範囲)
- SC-5 / SC-6 等他の SC: 構造不変、影響なし

**re_activations counter**: 本変更は Step 7 review-driven Blocker / Major findings ではなく **Step 7 → Step 8 transition での user-gate-driven structural refinement** のため、TODO.md の T-E / T-H / T-I の `re_activations` は **加算しない** (notes 欄に commit SHA 記録のみ)。

**実装完了 commits**:

- `refactor(dev-workflow/feed-platform-ms-01-workspace-foundation/T-E+T-H+T-I/r2): split smoke tests into feature/*.test.ts colocated with verification targets` (smoke.test.ts × 3 削除 + feature/\*.test.ts × 5 新規追加 + design.md / qa-design.md / task-plan.md / TODO.md / ADR-01 更新)
