...
// 後段で `Layer.provide(envLayer)` を内側にも重ねて dynamicLoggerLayer 自体の Env 依存を閉じる。
...
const makeRuntime = (env: Env.Type) => {
  ...
  // 全体に Layer.provide で重ねる (= Env を 2 箇所に provide する形式)。
...
> Note (Phase 1 deviation, 2026-05-06 記録): 上記 `makeRuntime` の Layer 合成形は本 design.md 初版から微調整されている。初版は `Layer.provide(dynamicLoggerLayer)` で外側に provide するだけだったが、`dynamicLoggerLayer` は内部で `yield* Env.Service` するため自身の Env 依存が閉じない。実装段階で `dynamicLoggerLayer.pipe(Layer.provide(envLayer))` の 2 重 provide 形 (= envLayer を内側にも明示的に provide) に修正し、3 プロジェクト同形で運用 (TODO.md T-B notes に deviation 経緯を記録)。動作仕様 (`env.ENV` で Logger 形式が切り替わる) は同一。
...
| 共通 `feature/env.ts` (`Env.Service` + `makeLayer`)        | 3 プロジェクトで **ほぼ完全同形コピー** (`Env.Type` の `ENV` フィールドのみ)                                        | **新規共通化マイルストーン (ms-02 認証着手前に挿入予定) でライブラリ化要検討**。`@app/<project>/feature/env/Service` の namespace 違いだけで実装は同じ。プロジェクト固有の env field が増えた場合は共通基盤 + プロジェクト個別 Layer の二段構成に拡張可能                 |
