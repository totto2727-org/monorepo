...
`js/package/auth/src/cookie-translator.ts` の `extractBearerFromCookie(cookieName, cookieHeader)` が `Cookie` ヘッダ文字列から `feed-session` cookie を抽出し `Authorization: Bearer <token>` 文字列を返す。`js/app/feed-platform-web/app/feature/api/client.ts` (BackendClient) がこの関数を呼び出し、バックエンドへの fetch に Authorization ヘッダを付与する。
...
- **Newly added:** `js/app/feed-platform-web/app/feature/auth/` に oauth-client.ts / callback.ts / middleware.ts / constants.ts が実装された。`js/app/feed-platform-backend/src/feature/auth/` に jwt.ts / middleware.ts が実装された。`js/package/auth/` に extractBearerFromCookie が実装された
...
- **Constraints going forward:**
  - `feed-session` cookie name は auth の `FEED_SESSION_COOKIE` 定数で一元管理。変更する場合は全 consumer の参照箇所を合わせて変更する
