...
objects/arrays of the above, JSX elements, and `<Frame>` elements. Functions and class instances cannot be passed.
...
During SSR, `handle.frame.src` points at the frame being rendered, while `handle.frames.top.src` stays fixed at the outer document URL.
