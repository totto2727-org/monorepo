...
- A foreign file, directory, or non-managed symlink at a desired output path is left untouched and reported as skipped.
...
- Test Path normalization, project/global/recursive discovery, local-source constraints, target deduplication, lock-scope key derivation, scope-metadata mismatch rejection, and cache isolation for two locks that pin the same repository differently.
...
- Existing directories and managed outputs are idempotent, and foreign paths are never overwritten or deleted.
