...
Run formatting, linting, and type checking through the Vite+ project tasks from the repository root:

```bash
vp run --filter @infra/aws fix
vp run --filter @infra/cloudflare fix
vp run --filter @infra/aws check
