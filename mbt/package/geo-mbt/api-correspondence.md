...
| `interiors_push` (`&mut self`)                                   | `Polygon::with_pushed_interior(Self, LineString) -> Self`                   | ✅     | Immutable counterpart: returns a new `Polygon` with the additional interior appended; does not mutate `self` and does not mutate the input `LineString` (it is cloned before the canonical constructor's auto-close). Implementation pre-sizes the destination interiors `Array` to its final length via `Array::makei` so there is no incremental `push` loop. |
...
constructor does not mutate input LineStrings, `with_pushed_interior`
