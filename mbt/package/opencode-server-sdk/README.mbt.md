...
```bash
moon run --target native mbt/package/opencode-server-sdk/src/examples/cli -- translate 'MoonBitでOpenCode SDKを実装しています。'
```

Fetch an HTTP or HTTPS page with OpenCode's `webfetch` tool and summarize it as Markdown:

```bash
moon run --target native mbt/package/opencode-server-sdk/src/examples/cli -- summarize-url https://example.com
```
